...
struct cudaGraphicsResource
    *cuda_vbo_resource;          // handles OpenGLES-CUDA exchange
...
void mat4f_Ortho(float left, float right, float bottom, float top, float near,
                 float far, matrix4 m) {
...
GLuint ShaderCreate(const char *vshader_filename,
    ...
    if (link_success == GL_FALSE) {
        ...
        char errorLog[maxLength];
        glGetShaderInfoLog(new_shaderprogram, maxLength, &maxLength,
                           &errorLog[0]);
...
void autoTest(char **argv) {
    ...
    if (!g_CheckRender->PPMvsPPM("fluidsGLES.ppm", ref_file, MAX_EPSILON_ERROR,
                                 0.25f)) {
        g_TotalErrors++;
    }
}
...
void keyboard(unsigned char key, int x, int y, int argc, char **argv) {
    switch (key) {
        ...
        case 27:
            g_bExitESC = true;
            cleanup();
            graphics_close_window();  // close window and destroy OpenGL ES
                                      // context
            return;
            break;
        case 'r':
...
int main(int argc, char **argv) {
    ...
    checkCudaErrors(cudaMemcpy(dvfield, hvfield, sizeof(cData) * DS,
                               cudaMemcpyHostToDevice));
