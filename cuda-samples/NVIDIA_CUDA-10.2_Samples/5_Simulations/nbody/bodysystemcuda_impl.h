...
void BodySystemCUDA<T>::_initialize(int numBodies) {
    ...
    for (unsigned int i = 0; i < m_numDevices; i++) {
        ...
        // Rounding up to numSms[i]*256 leads to better GPU utilization _per_
        // GPU but when using multiple devices, it will lead to the last GPUs
        // not having any work at all which means worse overall performance
    ...
    if (m_bUseSysMem) {
        checkCudaErrors(
            cudaHostAlloc((void **)&m_hPos[0], memSize,
                          cudaHostAllocMapped | cudaHostAllocPortable));
        checkCudaErrors(
            cudaHostAlloc((void **)&m_hPos[1], memSize,
                          cudaHostAllocMapped | cudaHostAllocPortable));
        checkCudaErrors(
            cudaHostAlloc((void **)&m_hVel, memSize,
                          cudaHostAllocMapped | cudaHostAllocPortable));
        ...
        for (unsigned int i = 0; i < m_numDevices; i++) {
            ...
            checkCudaErrors(cudaHostGetDevicePointer(
                (void **)&m_deviceData[i].dVel, (void *)m_hVel, 0));
        }
    } else {
        ...
        if (m_bUsePBO) {
            ...
            for (int i = 0; i < 2; ++i) {
                ...
                glBufferData(GL_ARRAY_BUFFER, memSize, m_hPos[0],
                             GL_DYNAMIC_DRAW);
                ...
                if ((unsigned)size != memSize) {
                    fprintf(stderr,
                            "WARNING: Pixel Buffer Object allocation failed!n");
                }

                glBindBuffer(GL_ARRAY_BUFFER, 0);
                checkCudaErrors(cudaGraphicsGLRegisterBuffer(
                    &m_pGRes[i], m_pbo[i], cudaGraphicsMapFlagsNone));
            }
        } else {
            checkCudaErrors(
                cudaMalloc((void **)&m_deviceData[0].dPos[0], memSize));
            checkCudaErrors(
                cudaMalloc((void **)&m_deviceData[0].dPos[1], memSize));
        }
    ...
    switch (array) {
        ...
        case BODYSYSTEM_POSITION: {
            if (m_bUsePBO) {
                ...
                glBufferSubData(GL_ARRAY_BUFFER, 0, 4 * sizeof(T) * m_numBodies,
                                data);
                ...
                if ((unsigned)size != 4 * (sizeof(T) * m_numBodies)) {
                    fprintf(stderr,
                            "WARNING: Pixel Buffer Object download failed!n");
                }

                glBindBuffer(GL_ARRAY_BUFFER, 0);
            } else {
                if (m_bUseSysMem) {
                    memcpy(m_hPos[m_currentRead], data,
                           m_numBodies * 4 * sizeof(T));
                } else
                    checkCudaErrors(cudaMemcpy(
                        m_deviceData[0].dPos[m_currentRead], data,
                        m_numBodies * 4 * sizeof(T), cudaMemcpyHostToDevice));
            }
        } break;
