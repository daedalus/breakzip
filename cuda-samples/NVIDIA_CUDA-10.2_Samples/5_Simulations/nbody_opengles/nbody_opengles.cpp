...
struct NBodyParams {
    ...
    void print() {
        printf("{ %f, %f, %f, %f, %f, %f, %f, %f, %f },\n", m_timestep,
               m_clusterScale, m_velocityScale, m_softening, m_damping,
               m_pointSize, m_x, m_y, m_z);
    }
};
...
public:
    ...
    static void display() {
        ...
        if (useHostMem) {
            // This event sync is required because we are rendering from the
            // host memory that CUDA is writing.  If we don't wait until CUDA is
            // done updating it, we will render partially updated data,
            // resulting in a jerky frame rate.
            if (!useCpu) {
...
private:
    ...
    void _reset(int numBodies, NBodyConfig config) {
        if (tipsyFile == "") {
            randomizeBodies(config, m_hPos, m_hVel, m_hColor,
                            ...
                            activeParams.m_velocityScale, numBodies, true);
    ...
    void _runBenchmark(int iterations) {
        ...
        printf("= %.3f %s-precision GFLOP/s at %d flops per interaction\n",
               gflops, (sizeof(T) > 4) ? "double" : "single",
               flopsPerInteraction);
    }
};
...
void initGL(int *argc, char **argv) {
    ...
    Window xWindow =
        XCreateWindow(xDisplay, xRootWindow, 0, 0, window_width, window_height,
                      0, CopyFromParent, InputOutput, CopyFromParent,
                      CWEventMask, &xCreateWindowAttributes);
    ...
    eglContext =
        eglCreateContext(eglDisplay, configList[0], NULL, contextAttrs);
    if (!eglContext) {
...
int main(int argc, char **argv) {
    ...
    if (!useCpu) {
        ...
        if (numDevsRequested > 1 && customGPU) {
            printf(
                "You can't use --numdevices and --device at the same time.\n");
            exit(EXIT_SUCCESS);
        }
    ...
    if (useCpu)
        ...
        numBodies =
            compareToCPU ? 4096 : blockSize * 4 * props.multiProcessorCount;
    } else {
        numBodies = 0;
        ...
        for (int i = 0; i < numDevsRequested; i++) {
            cudaDeviceProp props;
            checkCudaErrors(cudaGetDeviceProperties(&props, i));
            numBodies += blockSize * (props.major >= 2 ? 4 : 1) *
    ...
    if (checkCmdLineFlag(argc, (const char **)argv, "numbodies")) {
        numBodies =
            getCmdLineArgumentInt(argc, (const char **)argv, "numbodies");
        ...
        if (numBodies < 1) {
            ...
            printf(
                "Warning: \"number of bodies\" specified %d is not a multiple "
                "of %d.\n",
                numBodies, blockSize);
