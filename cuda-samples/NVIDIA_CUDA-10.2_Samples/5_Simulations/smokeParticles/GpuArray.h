...
void GpuArray<T>::allocVbo(bool useElementArray) {
    ...
    if (m_doubleBuffer) {
        ...
        checkCudaErrors(
            cudaGraphicsGLRegisterBuffer(&m_cuda_vbo_resource[1], m_vbo[1],
...
void GpuArray<T>::map() {
    if (m_vbo[0]) {
        checkCudaErrors(
            cudaGraphicsMapResources(1, &m_cuda_vbo_resource[0], 0));
        size_t num_bytes;
    ...
    if (m_doubleBuffer && m_vbo[1]) {
        checkCudaErrors(
            cudaGraphicsMapResources(1, &m_cuda_vbo_resource[1], 0));
        size_t num_bytes;
        checkCudaErrors(cudaGraphicsResourceGetMappedPointer(
...
void GpuArray<T>::unmap() {
    if (m_vbo[0]) {
        checkCudaErrors(
            cudaGraphicsUnmapResources(1, &m_cuda_vbo_resource[0], 0));
        m_dptr[0] = 0;
    }

    if (m_doubleBuffer && m_vbo[1]) {
        checkCudaErrors(
            cudaGraphicsUnmapResources(1, &m_cuda_vbo_resource[1], 0));
        m_dptr[1] = 0;
    }
}
