...
void display() {
    ...
    if (fpsCount >= fpsLimit) {
        ...
        sprintf(fps, "CUDA Smoke Particles (%d particles): %3.1f fps",
                numParticles, ifps);
    ...
    switch (mode) {
        case M_VIEW: {
            ...
            if (buttonState == 2) {
                // middle = translate
                vec3f v =
                    vec3f(dx * translateSpeed, -dy * translateSpeed, 0.0f);
                vec3f r;
...
void runEmitter() {
    ...
    psystem->sphereEmitter(emitterIndex, cursorPosLag, vel, spread,
                           emitterRadius, ftoi(emitterRate * timestep),
                           particleLifetime, particleLifetime * 0.1f);
...
void initParams() {
    ...
    SimParams &p = psystem->getParams();
    params->AddParam(new Param<float>("damping", 0.99f, 0.0f, 1.0f, 0.001f,
                                      &p.globalDamping));
    params->AddParam(new Param<float>("gravity", 0.0f, 0.01f, -0.01f, 0.0001f,
                                      &p.gravity.y));
    ...
    params->AddParam(new Param<float>("light color r", lightColor[0], 0.0f,
                                      1.0f, 0.01f, &lightColor[0]));
    params->AddParam(new Param<float>("light color g", lightColor[1], 0.0f,
                                      1.0f, 0.01f, &lightColor[1]));
    params->AddParam(new Param<float>("light color b", lightColor[2], 0.0f,
                                      1.0f, 0.01f, &lightColor[2]));

    params->AddParam(new Param<float>("atten color r", colorAttenuation[0],
                                      0.0f, 1.0f, 0.01f, &colorAttenuation[0]));
    params->AddParam(new Param<float>("atten color g", colorAttenuation[1],
                                      0.0f, 1.0f, 0.01f, &colorAttenuation[1]));
    params->AddParam(new Param<float>("atten color b", colorAttenuation[2],
                                      0.0f, 1.0f, 0.01f, &colorAttenuation[2]));
    ...
    params->AddParam(new Param<float>("emitter radius", emitterRadius, 0.0f,
                                      2.0f, 0.01f, &emitterRadius));
    params->AddParam(new Param<uint>("emitter rate", emitterRate, 0, 10000, 1,
                                     &emitterRate));
    params->AddParam(new Param<float>("emitter velocity", emitterVel, 0.0f,
                                      0.1f, 0.001f, &emitterVel));
    params->AddParam(new Param<float>("emitter spread", emitterSpread, 0.0f,
                                      0.1f, 0.001f, &emitterSpread));

    params->AddParam(new Param<float>("particle lifetime", particleLifetime,
                                      0.0f, 1000.0f, 1.0f, &particleLifetime));
}
...
int main(int argc, char **argv) {
    ...
    if (g_bQAReadback) {
        ...
        sdkDumpBin(pos, numParticles * sizeof(float4),
                   "smokeParticles_pos.bin");
        sdkDumpBin(vel, numParticles * sizeof(float4),
                   "smokeParticles_vel.bin");
        ...
        // 1st initialize OpenGL context, so we can properly set the GL for
        // CUDA. This is needed to achieve optimal performance with OpenGL/CUDA
        // interop.
        initGL(&argc, argv);
