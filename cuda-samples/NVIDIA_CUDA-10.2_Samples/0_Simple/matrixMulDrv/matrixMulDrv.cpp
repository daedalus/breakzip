...
void runTest(int argc, char **argv) {
    ...
    if (1) {
        ...
        if (use_64bit_memory_address &&
            ...
            void *args[5] = {&d_C, &d_A, &d_B, &Matrix_Width_A,
                             &Matrix_Width_B};
        ...
        *(reinterpret_cast<CUdeviceptr *>(&argBuffer[offset])) = d_B;
        ...
        if (use_64bit_memory_address &&
            ...
            *(reinterpret_cast<CUdeviceptr *>(&argBuffer[offset])) =
                Matrix_Width_A;
    ...
    checkCudaErrors(
        cuMemcpyDtoH(reinterpret_cast<void *>(h_C), d_C, mem_size_C));
...
static int initCUDA(int argc, char **argv, CUfunction *pMatrixMul) {
    ...
    if (!findModulePath(PTX_FILE, module_path, argv, ptx_source)) {
        if (!findModulePath(CUBIN_FILE, module_path, argv, ptx_source)) {
            printf(
                "> findModulePath could not find <matrixMul_kernel> ptx or "
                "cubin\n");
            exit(EXIT_FAILURE);
        }
    } else {
    ...
    if (module_path.rfind("ptx") != std::string::npos) {
        ...
        checkCudaErrors(cuModuleLoadDataEx(
            &cuModule, ptx_source.c_str(), jitNumOptions, jitOptions,
            reinterpret_cast<void **>(jitOptVals)));
