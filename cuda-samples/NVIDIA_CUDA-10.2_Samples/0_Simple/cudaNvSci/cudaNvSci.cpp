...
class cudaNvSciSignal
{
    ...
    cudaNvSciSignal(NvSciBufModule bufModule, NvSciSyncModule syncModule, int cudaDeviceId,
                    int bufSize, uint64_t imageWidth, uint64_t imageHeight, NvSciSyncFence *fence) :
                    m_syncModule(syncModule), m_bufModule(bufModule), m_cudaDeviceId(cudaDeviceId),
                    d_outputBuf(NULL), m_bufSize(bufSize), m_imageWidth(imageWidth), 
    ...
    void initCuda()
    {
        ...
        printf("[cudaNvSciSignal] GPU Device %d: \"%s\" with compute capability %d.%d\n\n",
    ...
    void setImageBufAttrList(uint32_t width, uint32_t height)
    {
        ...
        checkNvSciErrors(NvSciBufAttrListSetAttrs(m_imageBufAttrList, imgBufAttrs,
                sizeof(imgBufAttrs)/sizeof(NvSciBufAttrKeyValuePair)));
    }

    NvSciSyncAttrList getNvSciSyncAttrList()
    {
        return m_syncAttrList;
    }

    NvSciBufAttrList getNvSciRawBufAttrList()
    {
        return m_rawBufAttrList;
    }

    NvSciBufAttrList getNvSciImageBufAttrList()
    {
        return m_imageBufAttrList;
    }
    ...
    void cudaImportNvSciRawBuf(NvSciBufObj inputBufObj)
    {
        ...
        checkNvSciErrors(NvSciBufAttrListGetAttrs(m_buffAttrListOut[0], pairArrayOut, 2));
    ...
    void copyDataToImageArray(unsigned char *imageData)
    {
        ...
        checkCudaErrors(cudaMemcpy2DToArrayAsync(d_mipLevelArray, 0, 0, imageData, 
                                            m_imageWidth*sizeof(unsigned int), m_imageWidth*sizeof(unsigned int),
                                            m_imageHeight, cudaMemcpyHostToDevice, streamToRun));
    }
...
class cudaNvSciWait
{
    ...
    cudaNvSciWait(NvSciBufModule bufModule, NvSciSyncModule syncModule, int cudaDeviceId, 
                    int bufSize, NvSciSyncFence *fence) : 
                    m_bufModule(bufModule), m_syncModule(syncModule), m_cudaDeviceId(cudaDeviceId),
                     m_bufSize(bufSize), m_fence(fence)
    {
        initCuda();
        checkNvSciErrors(NvSciSyncAttrListCreate(m_syncModule, &m_syncAttrList));
        checkNvSciErrors(NvSciBufAttrListCreate(m_bufModule, &m_rawBufAttrList));
    ...
    void initCuda()
    {
        ...
        checkCudaErrors(cudaDeviceGetAttribute(&major, cudaDevAttrComputeCapabilityMajor, m_cudaDeviceId));
        checkCudaErrors(cudaDeviceGetAttribute(&minor, cudaDevAttrComputeCapabilityMinor, m_cudaDeviceId));
        printf("[cudaNvSciWait] GPU Device %d: \"%s\" with compute capability %d.%d\n\n",
               m_cudaDeviceId, _ConvertSMVer2ArchName(major, minor), major, minor);
    }
    ...
    void cudaImportNvSciRawBuf(NvSciBufObj inputBufObj)
    {
        ...
        checkNvSciErrors(NvSciBufAttrListGetAttrs(m_buffAttrListOut, pairArrayOut, 2));
...
void thread_waitAndGrayscale(cudaNvSciWait *cudaNvSciWaitObj, std::string image_filename, 
...
cudaNvSci::cudaNvSci(int isMultiGPU, std::vector<int> &deviceIds, unsigned char *imageData, size_t width, size_t height) : 
                    m_isMultiGPU(isMultiGPU), image_data(imageData), imageWidth(width), imageHeight(height) 
{
    if (isMultiGPU)
    {
...
void cudaNvSci::runCudaNvSci(std::string& image_filename)
{
    ...
    cudaNvSciSignal rotateAndSignal(buffModule, syncModule, m_cudaNvSciSignalDeviceId,
                                    m_bufSize, imageWidth, imageHeight, fence);
    cudaNvSciWait waitAndGrayscale(buffModule, syncModule, m_cudaNvSciWaitDeviceId,
                                    m_bufSize, fence);
    ...
    std::thread grayscaleThread(&thread_waitAndGrayscale, &waitAndGrayscale, image_filename,
...
{
    int numAttrList = 2;
    checkNvSciErrors(NvSciBufAttrListReconcile(rawBufUnreconciledList, numAttrList, 
                                                &rawBufReconciledList, &buffConflictList));
...
{
    int numAttrList = 1;
    checkNvSciErrors(NvSciBufAttrListReconcile(imageBufUnreconciledList, numAttrList, 
                                                &imageBufReconciledList, &imageBufConflictList));
...
void cudaNvSci::createNvSciSyncObj()
{
    ...
    checkNvSciErrors(NvSciSyncAttrListReconcile(syncUnreconciledList, numAttrList, 
                                                    &syncReconciledList, &syncConflictList));
