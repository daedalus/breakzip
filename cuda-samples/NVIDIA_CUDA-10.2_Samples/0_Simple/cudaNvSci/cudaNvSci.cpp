...
public:
    cudaNvSciSignal(NvSciBufModule bufModule, NvSciSyncModule syncModule,
        ...
        checkNvSciErrors(
            NvSciSyncAttrListCreate(m_syncModule, &m_syncAttrList));
        checkNvSciErrors(
            NvSciBufAttrListCreate(m_bufModule, &m_rawBufAttrList));
        checkNvSciErrors(
            NvSciBufAttrListCreate(m_bufModule, &m_imageBufAttrList));
    ...
    void setRawBufAttrList(uint64_t size) {
        ...
        NvSciBufAttrKeyValuePair rawBufAttrs[] = {
            ...
            {NvSciBufRawBufferAttrKey_Align, &tempAlignment,
             sizeof(tempAlignment)},
            {NvSciBufGeneralAttrKey_Types, &bufType, sizeof(bufType)},
            {NvSciBufGeneralAttrKey_NeedCpuAccess, &cpuAccess,
             sizeof(cpuAccess)},
    ...
    void runRotateImageAndSignal(unsigned char *imageData) {
        ...
        rotateKernel(texObject, angle, (unsigned int *)d_outputBuf,
                     m_imageWidth, m_imageHeight, streamToRun);
    ...
    void signalExternalSemaphore() {
        ...
        checkCudaErrors(cudaSignalExternalSemaphoresAsync(
            &signalSem, &signalParams, 1, streamToRun));
    }
    ...
    void cudaImportNvSciImage(NvSciBufObj inputBufObj) {
        ...
        memHandleDesc.size = size;
        checkCudaErrors(
            cudaImportExternalMemory(&extMemImageBuf, &memHandleDesc));
    ...
    void copyDataToImageArray(unsigned char *imageData) {
        ...
        checkCudaErrors(cudaGetMipmappedArrayLevel(&d_mipLevelArray,
                                                   d_mipmapArray, mipLevelId));

        checkCudaErrors(cudaMemcpy2DToArrayAsync(
            d_mipLevelArray, 0, 0, imageData,
...
public:
    ...
    void waitExternalSemaphore() {
        ...
        checkCudaErrors(cudaWaitExternalSemaphoresAsync(&waitSem, &waitParams,
                                                        1, streamToRun));
    }
    ...
    void cudaImportNvSciRawBuf(NvSciBufObj inputBufObj) {
        ...
        checkNvSciErrors(
            NvSciBufObjGetAttrList(inputBufObj, &m_buffAttrListOut));
...
void thread_waitAndGrayscale(cudaNvSciWait *cudaNvSciWaitObj,
    ...
    cudaNvSciWaitObj->runImageGrayscale(image_filename, imageWidth,
                                        imageHeight);
}
...
void cudaNvSci::createNvSciRawBufObj() {
    int numAttrList = 2;
    checkNvSciErrors(
        NvSciBufAttrListReconcile(rawBufUnreconciledList, numAttrList,
...
void cudaNvSci::createNvSciSyncObj() {
    int numAttrList = 2;
    checkNvSciErrors(
        NvSciSyncAttrListReconcile(syncUnreconciledList, numAttrList,
