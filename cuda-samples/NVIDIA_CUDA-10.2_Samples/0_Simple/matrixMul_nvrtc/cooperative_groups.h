...
class thread_group {
    ...
    friend _CG_QUALIFIER thread_group
    tiled_partition(const thread_group& parent, unsigned int tilesz);
...
public:
    ...
    _CG_QUALIFIER bool is_valid() const { return (_data.handle != 0); }
 ...
 * must have been launched with cuLaunchCooperativeKernel (or the CUDA Runtime
 * equivalent), and the device must support it (queryable device attribute).
 *
 ...
 */
_CG_QUALIFIER grid_group this_grid() { return (grid_group()); }
 ...
 * A thread_block represents a thread block whose dimensions are not known until
 * runtime.
 *
...
class thread_block : public thread_group {
    ...
    tiled_partition(const thread_group& parent, unsigned int tilesz);
    friend _CG_QUALIFIER thread_group
    tiled_partition(const thread_block& parent, unsigned int tilesz);
 ...
 */
_CG_QUALIFIER thread_block this_thread_block() { return (thread_block()); }
...
class coalesced_group : public thread_group {
    ...
    friend _CG_QUALIFIER thread_group
    tiled_partition(const thread_group& parent, unsigned int tilesz);
    friend _CG_QUALIFIER coalesced_group
    ...
    _CG_QUALIFIER unsigned int _packLanes(unsigned laneMask) const {
        ...
        for (int bit_idx = 0; bit_idx < 32; bit_idx++) {
            ...
            if (lane_bit) {
                if (laneMask & lane_bit) member_pack |= 1 << member_rank;
                member_rank++;
            }
        }
        return (member_pack);
    ...
    // Internal Use
    _CG_QUALIFIER coalesced_group
    _get_tiled_threads(unsigned int tilesz) const {
        const bool pow2_tilesz = ((tilesz & (tilesz - 1)) == 0);
...
protected:
    ...
    _CG_QUALIFIER coalesced_group(unsigned int mask)
        : thread_group(__internal::Coalesced) {
        _data.coalesced.mask = mask;
        _data.coalesced.size = __popc(mask);
    }

public:
    _CG_QUALIFIER unsigned int size() const { return (_data.coalesced.size); }
    _CG_QUALIFIER unsigned int thread_rank() const {
...
#define COALESCED_SHFL_FUNCTION(type)                                          \
    _CG_QUALIFIER type shfl(type var, unsigned int src_rank) const {           \
        unsigned int lane = (src_rank == 0)                                    \
                                ? __ffs(_data.coalesced.mask) - 1              \
                                : (size() == 32) ? src_rank                    \
                                                 : __fns(_data.coalesced.mask, \
                                                         0, (src_rank + 1));   \
...
#define COALESCED_SHFL_UP_FUNCTION(type)                                     \
    _CG_QUALIFIER type shfl_up(type var, int delta) const {                  \
        ...
        unsigned lane =                                                      \
            __fns(_data.coalesced.mask, __internal::laneid(), -(delta + 1)); \
...
#undef COALESCED_SHFL_DOWN_FUNCTION
    ...
    _CG_QUALIFIER unsigned int ballot(int predicate) const {
        ...
        unsigned int lane_ballot =
            __ballot_sync(_data.coalesced.mask, predicate);
...
#define COALESCED_MATCH_ALL_FUNCTION(type)                            \
    _CG_QUALIFIER unsigned int match_all(type val, int& pred) const { \
        ...
        unsigned int lane_match =                                     \
            __match_all_sync(_data.coalesced.mask, val, &pred);       \
        return (_packLanes(lane_match));                              \
    }
...
protected:
    _CG_QUALIFIER __thread_block_tile_base()
        : thread_group(__internal::CoalescedTile) {
...
public:
    ...
    _CG_QUALIFIER unsigned int shfl_down(unsigned int var,
                                         unsigned int delta) const {
        return (__shfl_down_sync(build_mask(), var, delta, numThreads));
    }
    _CG_QUALIFIER unsigned int shfl_up(unsigned int var,
                                       unsigned int delta) const {
        return (__shfl_up_sync(build_mask(), var, delta, numThreads));
    }
    _CG_QUALIFIER unsigned int shfl_xor(unsigned int var,
                                        unsigned int laneMask) const {
        return (__shfl_xor_sync(build_mask(), var, laneMask, numThreads));
    }
    ...
    _CG_QUALIFIER unsigned long shfl_down(unsigned long var,
                                          unsigned int delta) const {
        return (__shfl_down_sync(build_mask(), var, delta, numThreads));
    }
    _CG_QUALIFIER unsigned long shfl_up(unsigned long var,
                                        unsigned int delta) const {
        return (__shfl_up_sync(build_mask(), var, delta, numThreads));
    }
    _CG_QUALIFIER unsigned long shfl_xor(unsigned long var,
                                         unsigned int laneMask) const {
        return (__shfl_xor_sync(build_mask(), var, laneMask, numThreads));
    }
    ...
    _CG_QUALIFIER long long shfl_xor(long long var,
                                     unsigned int laneMask) const {
        return (__shfl_xor_sync(build_mask(), var, laneMask, numThreads));
    }
    _CG_QUALIFIER unsigned long long shfl(unsigned long long var,
                                          int srcRank) const {
        return (__shfl_sync(build_mask(), var, srcRank, numThreads));
    }
    _CG_QUALIFIER unsigned long long shfl_down(unsigned long long var,
                                               unsigned int delta) const {
        return (__shfl_down_sync(build_mask(), var, delta, numThreads));
    }
    _CG_QUALIFIER unsigned long long shfl_up(unsigned long long var,
                                             unsigned int delta) const {
        return (__shfl_up_sync(build_mask(), var, delta, numThreads));
    }
    _CG_QUALIFIER unsigned long long shfl_xor(unsigned long long var,
                                              unsigned int laneMask) const {
        return (__shfl_xor_sync(build_mask(), var, laneMask, numThreads));
    }
    ...
    _CG_QUALIFIER int any(int predicate) const {
        unsigned int lane_ballot =
            build_mask() & __ballot_sync(build_mask(), predicate);
    ...
    _CG_QUALIFIER unsigned int match_any(int val) const {
        unsigned int lane_match =
            build_mask() & __match_any_sync(build_mask(), val);
    ...
    _CG_QUALIFIER unsigned int match_all(int val, int& pred) const {
        unsigned int lane_match =
            build_mask() & __match_all_sync(build_mask(), val, &pred);
        return (lane_match >> (__internal::laneid() & (~(numThreads - 1))));
    }
    ...
    _CG_QUALIFIER unsigned int match_all(unsigned long long val,
                                         int& pred) const {
        unsigned int lane_match =
            build_mask() & __match_all_sync(build_mask(), val, &pred);
        return (lane_match >> (__internal::laneid() & (~(numThreads - 1))));
...
template <>
class thread_block_tile<32> : public __thread_block_tile_base<32> {};
template <>
class thread_block_tile<16> : public __thread_block_tile_base<16> {};
template <>
class thread_block_tile<8> : public __thread_block_tile_base<8> {};
template <>
class thread_block_tile<4> : public __thread_block_tile_base<4> {};
template <>
class thread_block_tile<2> : public __thread_block_tile_base<2> {};
template <>
class thread_block_tile<1> : public __thread_block_tile_base<1> {};
...
template <class GroupT>
_CG_QUALIFIER unsigned int thread_rank(GroupT const& g) {
    return (g.thread_rank());
}

template <class GroupT>
_CG_QUALIFIER unsigned int group_size(GroupT const& g) {
    return (g.size());
}
...
_CG_QUALIFIER void thread_group::sync() const {
    if (_data.type == __internal::Coalesced ||
        _data.type == __internal::CoalescedTile) {
 ...
 *
 * Returns the linearized rank of the calling thread along the interval [0,
 * size()).
 */
 ...
 * 32 threads. Only thread_block, thread_block_tile<>, and their subgroups can
 * be tiled_partition() in _CG_VERSION 1000.
 */
_CG_QUALIFIER thread_group tiled_partition(const thread_group& parent,
                                           unsigned int tilesz) {
    if (parent._data.type == __internal::Coalesced ||
        parent._data.type == __internal::CoalescedTile) {
        return (static_cast<const coalesced_group&>(parent)._get_tiled_threads(
            tilesz));
    } else {
        return (static_cast<const thread_block&>(parent)._get_tiled_threads(
            tilesz));
    }
}
// Thread block type overload: returns a basic thread_group for now (may be
// specialized later)
_CG_QUALIFIER thread_group tiled_partition(const thread_block& parent,
                                           unsigned int tilesz) {
    return (parent._get_tiled_threads(tilesz));
}
// Coalesced group type overload: retains its ability to stay coalesced
_CG_QUALIFIER coalesced_group tiled_partition(const coalesced_group& parent,
                                              unsigned int tilesz) {
    return (parent._get_tiled_threads(tilesz));
}
...
template <unsigned int Size>
struct tiled_partition_impl<Size, thread_block>
    : public thread_block_tile<Size> {
    _CG_QUALIFIER tiled_partition_impl(thread_block const&)
        : thread_block_tile<Size>() {}
};
template <unsigned int Size>
struct tiled_partition_impl<Size, thread_block_tile<32> >
    : public thread_block_tile<Size> {
    _CG_QUALIFIER tiled_partition_impl(thread_block_tile<32> const&)
        : thread_block_tile<Size>() {}
};
template <unsigned int Size>
struct tiled_partition_impl<Size, thread_block_tile<16> >
    : public thread_block_tile<Size> {
    _CG_QUALIFIER tiled_partition_impl(thread_block_tile<16> const&)
        : thread_block_tile<Size>() {}
};
template <unsigned int Size>
struct tiled_partition_impl<Size, thread_block_tile<8> >
    : public thread_block_tile<Size> {
    _CG_QUALIFIER tiled_partition_impl(thread_block_tile<8> const&)
        : thread_block_tile<Size>() {}
};
template <unsigned int Size>
struct tiled_partition_impl<Size, thread_block_tile<4> >
    : public thread_block_tile<Size> {
    _CG_QUALIFIER tiled_partition_impl(thread_block_tile<4> const&)
        : thread_block_tile<Size>() {}
};
template <unsigned int Size>
struct tiled_partition_impl<Size, thread_block_tile<2> >
    : public thread_block_tile<Size> {
    _CG_QUALIFIER tiled_partition_impl(thread_block_tile<2> const&)
        : thread_block_tile<Size>() {}
};
template <>
struct tiled_partition_impl<1, thread_block_tile<1> >
    : public thread_block_tile<1> {
    _CG_QUALIFIER tiled_partition_impl(thread_block_tile<1> const&)
        : thread_block_tile<1>() {}
};
