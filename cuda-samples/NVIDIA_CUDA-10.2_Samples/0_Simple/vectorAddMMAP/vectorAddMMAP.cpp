...
vector<CUdevice> getBackingDevices(CUdevice cuDevice) {
    ...
    for (int dev = 0; dev < num_devices; dev++) {
        ...
        // The device needs to support virtual address management for the
...
int main(int argc, char **argv) {
    ...
    // cuMemSetAccess is still subject to the constraints of
    // cuDeviceCanAccessPeer for cross device mappings (hence why we checked
    // cuDeviceCanAccessPeer earlier).
    checkCudaErrors(simpleMallocMultiDeviceMmap(
        &d_A, &allocationSize, size, backingDevices, mappingDevices));
    checkCudaErrors(simpleMallocMultiDeviceMmap(
        &d_B, NULL, size, backingDevices, mappingDevices));
    checkCudaErrors(simpleMallocMultiDeviceMmap(
        &d_C, NULL, size, backingDevices, mappingDevices));
