...
CUresult simpleMallocMultiDeviceMmap(
    ...
    // will be allocated via cuMemCreate allong with additional properties In
    // this case, the allocation will be pinnded device memory local to a given
    // device.
    ...
    {
        // Each accessDescriptor will describe the mapping requirement for a
        // single device
        std::vector<CUmemAccessDesc> accessDescriptors;
        accessDescriptors.resize(mappingDevices.size());

        // Prepare the access descriptor array indicating where and how the
        // backings should be visible.
        for (size_t idx = 0; idx < mappingDevices.size(); idx++) {
