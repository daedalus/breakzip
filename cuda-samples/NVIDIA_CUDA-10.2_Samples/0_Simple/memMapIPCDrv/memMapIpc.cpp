...
static void memMapAllocateAndExportMemory(
    ...
    if (allocSize % granularity) {
        printf(
            "Allocation size is not a multiple of minimum supported "
            "granularity for this device. Exiting...\n");
        exit(EXIT_FAILURE);
    }

    // Windows-specific LPSECURITYATTRIBUTES is required when
    // CU_MEM_HANDLE_TYPE_WIN32 is used. The security attribute defines the
    // scope of which exported allocations may be tranferred to other processes.
    // For all other handle types, pass NULL.
    ...
    for (int i = 0; i < allocationHandles.size(); i++) {
        ...
        // prop.location.id
        checkCudaErrors(
            cuMemCreate(&allocationHandles[i], allocSize, &prop, 0));

        // Export the allocation to a platform-specific handle. The type of
        // handle requested here must match the requestedHandleTypes field in
        // the prop structure passed to cuMemCreate.
        checkCudaErrors(cuMemExportToShareableHandle(
            (void *)&shareableHandles[i], allocationHandles[i],
            ipcHandleTypeFlag, 0));
    }
}
...
static void memMapImportAndMapMemory(
    ...
    for (int i = 0; i < shareableHandles.size(); i++) {
        // Import the memory allocation back into a CUDA handle from the
        ...
        checkCudaErrors(cuMemMap(d_ptr + (i * mapSize), mapSize, 0,
                                 allocationHandles[i], 0));

        // Since we do not need to make any other mappings of this memory or
        // export it, we no longer need and can release the allocationHandle.
...
static void memMapUnmapAndFreeMemory(CUdeviceptr dptr, size_t size) {
    ...
    // Since the memory has been unmapped after this call, accessing the
    // specified va range will result in a fault (unitll it is remapped).
...
static void memMapGetDeviceFunction(char **argv) {
    ...
    if (!findModulePath(PTX_FILE, module_path, argv, ptx_source)) {
        if (!findModulePath("memMapIpc_kernel.cubin", module_path, argv,
                            ptx_source)) {
            printf(
                "> findModulePath could not find <simpleMemMapIpc> ptx or "
                "cubin\n");
            exit(EXIT_FAILURE);
        }
    } else {
...
static void childProcess(int id, char **argv) {
    ...
    // Import the memory allocations shared by the parent with us and map them
    ...
    for (int i = 0; i < procCount; i++) {
        ...
        checkCudaErrors(cuLaunchKernel(_memMapIpc_kernel, blocks, 1, 1, threads,
                                       1, 1, 0, stream, args, 0));
    ...
    checkCudaErrors(cuMemcpyDtoHAsync(&verification_buffer[0],
                                      d_ptr + (id * DATA_BUF_SIZE),
                                      DATA_BUF_SIZE, stream));
    checkCudaErrors(cuStreamSynchronize(stream));
    ...
    for (unsigned long long j = 0; j < DATA_BUF_SIZE; j++) {
        if (verification_buffer[j] != compareId) {
            printf("Process %d: Verification mismatch at %lld: %d != %d\n", id,
                   j, (int)verification_buffer[j], (int)compareId);
            break;
        }
    }
    ...
    // Unmap the allocations from our address space. Unmapping will also free
    // the handle as we have already released the imported handle with the call
    // to cuMemRelease. Finally, free up the Virtual Address space we reserved
    // with cuMemAddressReserve.
...
static void parentProcess(char *app) {
    ...
    for (i = 0; i < devCount; i++) {
        ...
        for (int j = 0; j < nprocesses; j++) {
            ...
            checkCudaErrors(cuDeviceCanAccessPeer(&canAccessPeerJI, devices[j],
                                                  devices[i]));
            checkCudaErrors(cuDeviceCanAccessPeer(&canAccessPeerIJ, devices[i],
                                                  devices[j]));
        ...
        if (allPeers) {
            ...
            // peers per GPU at a time, this acts to remove devices from
            // CanAccessPeer
            for (int j = 0; j < nprocesses; j++) {
    ...
    // Allocate `nprocesses` number of memory chunks and obtain a shareable
    // handle for each allocation. Share all memory allocations with all
    // children.
