...
void runTest(int argc, char **argv) {
    ...
    if (checkCmdLineFlag(argc, (const char **)argv, "regression")) {
        ...
        sdkWriteFile<float>("./data/regression.dat", h_odata, width * height,
                            0.0f, false);
    } else {
        // We need to reload the data from disk, because it is inverted upon
        // output
