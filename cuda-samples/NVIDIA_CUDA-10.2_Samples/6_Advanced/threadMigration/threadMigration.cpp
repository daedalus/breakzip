...
static CUresult InitCUDAContext(CUDAContext *pContext, CUdevice hcuDevice,
    ...
    if (!findModulePath(PTX_FILE, module_path, argv, ptx_source)) {
        if (!findModulePath(CUBIN_FILE, module_path, argv, ptx_source)) {
            fprintf(stderr,
                    "> findModulePath could not find <threadMigration> ptx or "
                    "cubin\n");
            CLEANUP_ON_ERROR(dptr, hcuModule, hcuContext, status);
        }
    }
    ...
    if (module_path.rfind(".ptx") != string::npos) {
        ...
        status =
            cuModuleLoadDataEx(&hcuModule, ptx_source.c_str(), jitNumOptions,
...
#endif
{
    ...
    printf(
        "<CUDA Device=%d, Context=%p, Thread=%d> - ThreadProc() Launched...\n",
    ...
    if (1) {
        ...
        status = cuLaunchKernel(pParams->hcuFunction, 1, 1, 1, 32, 1, 1, 0,
                                NULL, args, NULL);
        ...
        *((CUdeviceptr *)&argBuffer[offset]) = pParams->dptr;
        ...
        // new CUDA 4.0 Driver API Kernel launch call
        status = cuLaunchKernel(pParams->hcuFunction, 1, 1, 1, 32, 1, 1, 0, 0,
    ...
    if (CUDA_SUCCESS ==
        ...
        for (int i = 0; i < NUM_INTS; i++) {
            if (pInt[i] != 32 - i) {
                printf(
                    "<CUDA Device=%d, Context=%p, Thread=%d> error [%d]=%d!\n",
                    pParams->deviceID, pParams->hcuContext, pParams->threadNum,
                    i, pInt[i]);
                wrong++;
            }
    ...
    printf(
        "<CUDA Device=%d, Context=%p, Thread=%d> - ThreadProc() Finished!\n\n",
        pParams->deviceID, pParams->hcuContext, pParams->threadNum);
...
bool runTest(int argc, char **argv) {
    ...
    if (argc > 1) {
        ...
        if (checkCmdLineFlag(argc, (const char **)argv, "numthreads")) {
            ...
            if (NumThreads < 1 || NumThreads > 15) {
                printf(
                    "Usage: \"threadMigration -n=<threads>\", <threads> ranges "
                    "1-15\n");
                return 1;
            }
        }
    ...
    for (int iDevice = 0; iDevice < deviceCount; iDevice++) {
...

        {
            ...
            checkCudaErrors(cuDeviceGetAttribute(
                &sharedMemPerBlock,
                CU_DEVICE_ATTRIBUTE_MAX_SHARED_MEMORY_PER_BLOCK, hcuDevice));
        ...
        if (CUDA_SUCCESS !=
            ...
            for (int iThread = 0; iThread < NumThreads; iThread++, ihThread++) {
                g_ThreadParams[ThreadIndex].hcuContext =
                    pContext[iDevice].hcuContext;
                g_ThreadParams[ThreadIndex].hcuModule =
                    pContext[iDevice].hcuModule;
                g_ThreadParams[ThreadIndex].hcuFunction =
                    pContext[iDevice].hcuFunction;
                g_ThreadParams[ThreadIndex].deviceID =
                    pContext[iDevice].deviceID;
                g_ThreadParams[ThreadIndex].threadNum = iThread;
                // Launch (NumThreads) for each CUDA context
#if defined(WIN32) || defined(_WIN32) || defined(WIN64) || defined(_WIN64)
                rghThreads[ThreadIndex] =
                    CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)ThreadProc,
                                 &g_ThreadParams[ThreadIndex], 0,
