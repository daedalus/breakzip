...
int main(int argc, char **argv) {
    ...
    for (uint arrayLength = 64; arrayLength <= N; arrayLength *= 2) {
        ...
        if (arrayLength == N) {
            double dTimeSecs =
                1.0e-3 * sdkGetTimerValue(&hTimer) / numIterations;
            printf(
                "sortingNetworks-bitonic, Throughput = %.4f MElements/s, Time "
                "= %.5f s, Size = %u elements, NumDevsUsed = %u, Workgroup = "
                "%u\n",
                (1.0e-6 * (double)arrayLength / dTimeSecs), dTimeSecs,
                arrayLength, 1, threadCount);
        }
        ...
        int valuesFlag =
            validateValues(h_OutputKeyGPU, h_OutputValGPU, h_InputKey,
