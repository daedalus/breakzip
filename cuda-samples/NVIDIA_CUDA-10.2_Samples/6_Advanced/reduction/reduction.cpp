        ...
        if (cpuFinalReduction) {
            ...
            while (s > cpuFinalThreshold) {
                ...
                getNumBlocksAndThreads(kernel, s, maxBlocks, maxThreads, blocks,
                                       threads);
                cudaMemcpy(d_idata, d_odata, s * sizeof(T),
        ...
        for (int i = minN; i <= maxN; i *= 2) {
            ...
            if (numBlocks <= MAX_BLOCK_DIM_SIZE) {
                benchmarkReduce(i, numThreads, numBlocks, maxThreads, maxBlocks,
                                kernel, testIterations, false, 1, timer,
...
bool runTest(int argc, char **argv, ReduceType datatype) {
    ...
    if (checkCmdLineFlag(argc, (const char **)argv, "maxblocks")) {
        maxBlocks =
            getCmdLineArgumentInt(argc, (const char **)argv, "maxblocks");
    }
    ...
    if (runShmoo) {
        ...
        for (int i = 0; i < size; i++) {
            // Keep the numbers small so we don't get truncation error in the
            // sum
            if (datatype == REDUCE_INT) {
                h_idata[i] = (T)(rand() & 0xFF);
            } else {
        ...
        int numBlocks = 0;
        int numThreads = 0;
        getNumBlocksAndThreads(whichKernel, size, maxBlocks, maxThreads,
        ...
        gpu_result = benchmarkReduce<T>(size, numThreads, numBlocks, maxThreads,
                                        maxBlocks, whichKernel, testIterations,
                                        cpuFinalReduction, cpuFinalThreshold,
                                        timer, h_odata, d_idata, d_odata);
