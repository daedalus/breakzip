        ...
        if (i < nA) {
            ...
            ranksB[(segmentBase + 0) / SAMPLE_STRIDE + i] =
                binarySearchExclusive(srcKey[segmentBase + i * SAMPLE_STRIDE],
                                      srcKey + segmentBase + stride, lenB,
                                      sortDir);
        }

        if (i < nB) {
            ranksB[(segmentBase + stride) / SAMPLE_STRIDE + i] =
                i * SAMPLE_STRIDE;
        ...
        if (i < nA) {
            uint dstPosA =
                binarySearchExclusive(
                    ...
                    ranks + (segmentBase + stride) / SAMPLE_STRIDE, nB, 1) +
                i;
...
static void merge(uint *dstKey, uint *dstVal, uint *srcAKey, uint *srcAVal,
    ...
    for (uint i = 0; i < lenA; i++) {
        uint dstPos =
            binarySearchExclusive(srcAKey[i], srcBKey, lenB, sortDir) + i;
    ...
    for (uint i = 0; i < lenB; i++) {
        uint dstPos =
            binarySearchInclusive(srcBKey[i], srcAKey, lenA, sortDir) + i;
        assert(dstPos < lenA + lenB);
...
extern "C" void mergeSortHost(uint *dstKey, uint *dstVal, uint *bufKey,
    ...
    for (uint stride = SHARED_SIZE_LIMIT; stride < N; stride <<= 1) {
        ...
        mergeElementaryIntervals(okey, oval, ikey, ival, limitsA, limitsB,
                                 stride, N, sortDir);

        if (lastSegmentElements <= stride) {
            // Last merge segment consists of a single array which just needs to
            // be passed through
            memcpy(okey + (N - lastSegmentElements),
                   ikey + (N - lastSegmentElements),
                   lastSegmentElements * sizeof(uint));
            memcpy(oval + (N - lastSegmentElements),
