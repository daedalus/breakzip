...
int main(int argc, char **argv) {
    ...
    if (checkCmdLineFlag(argc, (const char **)argv, "gpumethod")) {
        gpumethod =
            getCmdLineArgumentInt(argc, (const char **)argv, "gpumethod");

        if (gpumethod < 0 || gpumethod > 2) {
            printf(
                "Error: gpumethod must be 0 or 1 or 2, gpumethod=%d is "
                "invalid\n",
                gpumethod);
            exit(EXIT_SUCCESS);
        }
    ...
    if (gpumethod == 0) {
        ...
        sumGPU = JacobiMethodGpu(d_A, d_b, conv_threshold, max_iter, d_x,
                                 d_x_new, stream1);
    }
