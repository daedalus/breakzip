...
int main(int argc, char *argv[]) {
    ...
    try {
        ...
        std::string sResultXFilename =
            sOutputDir + sFilename +
            "_gradientVectorPrewittBorderX_Vertical.pgm";
        ...
        //            "output", &outputFilePath); sResultBaseFilename =
        //            outputFilePath;
        //        }
        ...
        // convert 16s_C1 result images to binary 8u_C1 output images using
        // constant value to adjust amount of visible detail
        ...
        // now use the Prewitt gradient border filter function in such a way
        // that no border replication operations will be applied
        ...
        // copy and enlarge the original device source image and surround it
        // with a white edge (border)
        ...
        // adjust oEnlargedDeviceSrc pixel pointer to point to the first pixel
        // of the original source image in the enlarged source image
        const Npp8u *pTemp =
        ...
        // tell the filter function what cartesian pixel position pAdjustedSrc
        // is pointing to within the enlarged source image
        ...
        NPP_CHECK_NPP(nppiGradientVectorPrewittBorder_8u16s_C1R(
            pAdjustedSrc, oEnlargedDeviceSrc.pitch(), oEnlargedSrcSize,
            oSrcOffset, oDeviceDstX.data(), oDeviceDstX.pitch(),
            oDeviceDstY.data(), oDeviceDstY.pitch(), 0, 0, 0, 0, oSizeROI,
            NPP_MASK_SIZE_3_X_3, nppiNormL1, NPP_BORDER_REPLICATE));

        // convert 16s_C1 result images to binary 8u_C1 output images using
        // constant value to adjust amount of visible detail
        NPP_CHECK_NPP(nppiCompareC_16s_C1R(
            oDeviceDstX.data(), oDeviceDstX.pitch(), 32,
            oDeviceDstOutXNoBorders.data(), oDeviceDstOutXNoBorders.pitch(),
            oSizeROI, NPP_CMP_GREATER_EQ));

        NPP_CHECK_NPP(nppiCompareC_16s_C1R(
            oDeviceDstY.data(), oDeviceDstY.pitch(), 32,
        ...
        // diff the two 8u_C1 result images one with and one without border
        // control
        ...
        // if you closely examine the above difference files (recommend using
        // GIMP for viewing using scaling with no interpolation) you will see
        // several single pixel differences (white pixels) along the right and
        // bottom edges of the default vs. borderless images this happens
        // because border pixels in the original source image are duplicated
        // when the filter kernels overlap the edge of the source image when
        // using the first version of the filter call but are actually sampled
        // from the enlarged source image when using the second version of the
        // filter call the technique used in the second filter call can be used
        // with any filter border function in NPP to duplicate results that
        // would be generated from a non-border filter function call by filling
        // the border pixel outside the embedded source image with the
        // appropriate border pixel values

        // here is how to use border control to process a source image in
        // multiple calls and get correct output in the destination image
        ...
        // enable border replication on the right and bottom borders of the
        ...
        // image will be generated however since oEnlargedSrcSize.width is still
        // set to oSrcSize.width then border control will be disabled when the
        // filter needs to access source pixels beyond the right side of the
        ...
        // run Prewitt edge detection gradient vector filter to generate the
        // left side of the output image
        NPP_CHECK_NPP(nppiGradientVectorPrewittBorder_8u16s_C1R(
            pAdjustedSrc, oEnlargedDeviceSrc.pitch(), oEnlargedSrcSize,
            oSrcOffset, oDeviceDstX.data(), oDeviceDstX.pitch(),
            oDeviceDstY.data(), oDeviceDstY.pitch(), 0, 0, 0, 0, oSizeROI,
        ...
        // and adjust the source offset parameter accordingly - this will in
        // effect turn off border control for the left border allowing the
        // necessary source pixels to be used
        ...
        // run Prewitt edge detection gradient vector filter to generate the
        // right side of the output image adjusting the destination image
        // pointers appropriately
        NPP_CHECK_NPP(nppiGradientVectorPrewittBorder_8u16s_C1R(
            pAdjustedSrc, oEnlargedDeviceSrc.pitch(), oEnlargedSrcSize,
            oSrcOffset, oDeviceDstX.data() + nLeftWidth, oDeviceDstX.pitch(),
        ...
        NPP_CHECK_NPP(nppiCompareC_16s_C1R(
            ...
            oDeviceDstOutXMixedBorders.pitch(), oSizeROI, NPP_CMP_GREATER_EQ));

        NPP_CHECK_NPP(nppiCompareC_16s_C1R(
            oDeviceDstY.data(), oDeviceDstY.pitch(), 32,
            oDeviceDstOutYMixedBorders.data(),
            oDeviceDstOutYMixedBorders.pitch(), oSizeROI, NPP_CMP_GREATER_EQ));
        // create additional output files
        ...
        std::cout << "Saved image: " << sResultXMixedBordersFilename
                  << std::endl;
        saveImage(sResultYMixedBordersFilename, oHostDstY);
        std::cout << "Saved image: " << sResultYMixedBordersFilename
                  << std::endl;

        // diff the original 8u_C1 result images with border control and the
        // mixed border control images, they should match (diff image will be
        // all black)

        NPP_CHECK_NPP(nppiAbsDiff_8u_C1R(
            oDeviceDstOutXMixedBorders.data(),
            oDeviceDstOutXMixedBorders.pitch(), oDeviceDstOutX.data(),
            oDeviceDstOutX.pitch(), oDeviceDstOutXDiff.data(),
            oDeviceDstOutXDiff.pitch(), oSizeROI));

        NPP_CHECK_NPP(nppiAbsDiff_8u_C1R(
            oDeviceDstOutYMixedBorders.data(),
            oDeviceDstOutYMixedBorders.pitch(), oDeviceDstOutY.data(),
            oDeviceDstOutY.pitch(), oDeviceDstOutYDiff.data(),
            oDeviceDstOutYDiff.pitch(), oSizeROI));
