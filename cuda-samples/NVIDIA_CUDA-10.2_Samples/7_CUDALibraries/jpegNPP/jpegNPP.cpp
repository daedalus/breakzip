...
void readFrameHeader(const unsigned char *pData, FrameHeader &header) {
    ...
    for (int c = 0; c < header.nComponents; ++c) {
        ...
        header.aQuantizationTableSelector[c] =
...
void writeFrameHeader(const FrameHeader &header, unsigned char *&pData) {
    ...
    for (int c = 0; c < header.nComponents; ++c) {
        ...
        writeAndAdvance<unsigned char>(pTemp,
                                       header.aQuantizationTableSelector[c]);
    }
...
int main(int argc, char **argv) {
    ...
    if (checkCmdLineFlag(argc, (const char **)argv, "scale")) {
        nScaleFactor =
            max(0.0f,
                min(getCmdLineArgumentFloat(argc, (const char **)argv, "scale"),
                    1.0f));
    } else {
        nScaleFactor = 0.5f;
    }
    ...
    while (nMarker != -1) {
        ...
        if ((nMarker == 0x0C0) | (nMarker == 0x0C2)) {
            ...
            // Note: NPP does support progressive jpegs for both encode and
            // decode
            if (nMarker != 0x0C0) {
                cerr << "The sample does only support baseline JPEG images"
                     << endl;
                return EXIT_SUCCESS;
            }
            ...
            if (oFrameHeader.nComponents != 3) {
                cerr << "The sample does only support color JPEG images"
                     << endl;
                return EXIT_SUCCESS;
            }

            // Compute channel sizes as stored in the JPEG (8x8 blocks & MCU
            // block layout)
            for (int i = 0; i < oFrameHeader.nComponents; ++i) {
                nMCUBlocksV =
                    max(nMCUBlocksV, oFrameHeader.aSamplingFactors[i] & 0x0f);
                nMCUBlocksH =
                    max(nMCUBlocksH, oFrameHeader.aSamplingFactors[i] >> 4);
            }
            ...
            for (int i = 0; i < oFrameHeader.nComponents; ++i) {
                ...
                aSrcActualSize[i].width = DivUp(
                    oFrameHeader.nWidth * oBlocksPerMCU.width, nMCUBlocksH);
                aSrcActualSize[i].height = DivUp(
                    oFrameHeader.nHeight * oBlocksPerMCU.height, nMCUBlocksV);

                oBlocks.width = (int)ceil(
                    (oFrameHeader.nWidth + 7) / 8 *
                    static_cast<float>(oBlocksPerMCU.width) / nMCUBlocksH);
                oBlocks.width = DivUp(oBlocks.width, oBlocksPerMCU.width) *
                                oBlocksPerMCU.width;

                oBlocks.height = (int)ceil(
                    (oFrameHeader.nHeight + 7) / 8 *
                    static_cast<float>(oBlocksPerMCU.height) / nMCUBlocksV);
                oBlocks.height = DivUp(oBlocks.height, oBlocksPerMCU.height) *
                                 oBlocksPerMCU.height;
                ...
                // Allocate Memory
                size_t nPitch;
                NPP_CHECK_CUDA(cudaMallocPitch(
                    &apdDCT[i], &nPitch, oBlocks.width * 64 * sizeof(Npp16s),
                    oBlocks.height));
                ...
                NPP_CHECK_CUDA(cudaHostAlloc(&aphDCT[i],
                                             aDCTStep[i] * oBlocks.height,
        ...
        if (nMarker == 0x0DA) {
            ...
            if (nRestartInterval > 0) {
                while (nAfterScanMarker >= 0x0D0 && nAfterScanMarker <= 0x0D7) {
                    ...
                    nAfterScanMarker = nextMarker(
                        pJpegData, nAfterNextMarkerPos, nInputLength);
                }
            }
            ...
            for (int i = 0; i < 3; ++i) {
                nppiDecodeHuffmanSpecInitAllocHost_JPEG(
                    pHuffmanDCTables[(oScanHeader.aHuffmanTablesSelector[i] >>
                                      4)]
                        .aCodes,
                    nppiDCTable, &apDecHuffmanDCTable[i]);
                nppiDecodeHuffmanSpecInitAllocHost_JPEG(
                    pHuffmanACTables[(oScanHeader.aHuffmanTablesSelector[i] &
                                      0x0f)]
                        .aCodes,
                    nppiACTable, &apDecHuffmanACTable[i]);
            }

            NPP_CHECK_NPP(nppiDecodeHuffmanScanHost_JPEG_8u16s_P3R(
                pJpegData + nPos, nAfterNextMarkerPos - nPos - 2,
                nRestartInterval, oScanHeader.nSs, oScanHeader.nSe,
                oScanHeader.nA >> 4, oScanHeader.nA & 0x0f, aphDCT, aDCTStep,
                apDecHuffmanDCTable, apDecHuffmanACTable, aSrcSize));
    ...
    Npp8u aZigzag[] = {0,  1,  5,  6,  14, 15, 27, 28, 2,  4,  7,  13, 16,
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        NPP_CHECK_CUDA(
            cudaMemcpyAsync((unsigned char *)pdQuantizationTables + i * 64,
                            temp, 64, cudaMemcpyHostToDevice));
    }
    ...
    for (int i = 0; i < 3; ++i) {
        NPP_CHECK_NPP(nppiDCTQuantInv8x8LS_JPEG_16s8u_C1R_NEW(
            ...
            pdQuantizationTables +
                oFrameHeader.aQuantizationTableSelector[i] * 64,
    ...
    if (aSrcActualSize[0].width == aSrcActualSize[1].width) {
        ...
        } else if (abs(static_cast<float>(aSrcActualSize[0].height -
                                          2 * aSrcActualSize[1].height)) < 3) {
            eComponentSampling = YCbCr_440;
        }
    } else if (abs(static_cast<float>(aSrcActualSize[0].width -
                                      2 * aSrcActualSize[1].width)) < 3) {
        if (aSrcActualSize[0].height == aSrcActualSize[1].height) {
    ...
    if (eComponentSampling == YCbCr_UNKNOWN) {
        cout << "invalid image - Y:" << aSrcActualSize[0].width << "x"
             << aSrcActualSize[0].height << "  Cb:" << aSrcActualSize[1].width
             << "x" << aSrcActualSize[1].height
             << "  Cr:" << aSrcActualSize[2].width << "x"
             << aSrcActualSize[2].height;
        return EXIT_FAILURE;
    }

    // Set sampling factor for destination image.
    oFrameHeader.aSamplingFactors[1] =
        (1 << 4) | (oFrameHeader.aSamplingFactors[1] & 0x0f);
    oFrameHeader.aSamplingFactors[1] =
        (oFrameHeader.aSamplingFactors[1] & 0xf0) | 1;
    oFrameHeader.aSamplingFactors[2] =
        (1 << 4) | (oFrameHeader.aSamplingFactors[2] & 0x0f);
    oFrameHeader.aSamplingFactors[2] =
        (oFrameHeader.aSamplingFactors[2] & 0xf0) | 1;

    switch (eComponentSampling) {
        case YCbCr_444: {
            // Y
            oFrameHeader.aSamplingFactors[0] =
                (1 << 4) | (oFrameHeader.aSamplingFactors[0] & 0x0f);
            oFrameHeader.aSamplingFactors[0] =
                (oFrameHeader.aSamplingFactors[0] & 0xf0) | 1;
            break;
        }
        case YCbCr_440: {
            // Y
            oFrameHeader.aSamplingFactors[0] =
                (1 << 4) | (oFrameHeader.aSamplingFactors[0] & 0x0f);
            oFrameHeader.aSamplingFactors[0] =
                (oFrameHeader.aSamplingFactors[0] & 0xf0) | 2;
            break;
        }
        case YCbCr_422: {
            // Y
            oFrameHeader.aSamplingFactors[0] =
                (2 << 4) | (oFrameHeader.aSamplingFactors[0] & 0x0f);
            oFrameHeader.aSamplingFactors[0] =
                (oFrameHeader.aSamplingFactors[0] & 0xf0) | 1;
            break;
        }
        ...
        case YCbCr_411: {
            // Y
            oFrameHeader.aSamplingFactors[0] =
                (4 << 4) | (oFrameHeader.aSamplingFactors[0] & 0x0f);
            oFrameHeader.aSamplingFactors[0] =
                (oFrameHeader.aSamplingFactors[0] & 0xf0) | 1;
            break;
        }
    ...
    for (int i = 0; i < oFrameHeader.nComponents; ++i) {
        ...
        // Allocate Memory
        size_t nPitch;
        NPP_CHECK_CUDA(cudaMallocPitch(&apDstImage[i], &nPitch,
                                       aDstSize[i].width, aDstSize[i].height));
    ...
    for (int i = 0; i < 3; ++i) {
        NppiSize oBlocksPerMCU = {oFrameHeader.aSamplingFactors[i] >> 4,
                                  oFrameHeader.aSamplingFactors[i] & 0x0f};
        NppiSize oSrcImageSize = {aSrcActualSize[i].width,
                                  aSrcActualSize[i].height};
        NppiRect oSrcImageROI = {0, 0, oSrcImageSize.width,
                                 oSrcImageSize.height};
        NppiRect oDstImageROI;
        oDstImageROI.x = 0;
        oDstImageROI.y = 0;
        oDstImageROI.width =
            DivUp(oDstImageSize.width * oBlocksPerMCU.width, nMCUBlocksH);
        oDstImageROI.height =
            DivUp(oDstImageSize.height * oBlocksPerMCU.height, nMCUBlocksV);
        ...
        int nBufferSize;
        NPP_CHECK_NPP(nppiResizeAdvancedGetBufferHostSize_8u_C1R(
            oSrcImageSize, oDstImageSize, &nBufferSize,
            NPPI_INTER_LANCZOS3_ADVANCED));
        NPP_CHECK_CUDA(cudaMalloc(&apLanczosBuffer[i], nBufferSize));
        NPP_CHECK_NPP(nppiResizeSqrPixel_8u_C1R_Advanced(
            apSrcImage[i], oSrcImageSize, aSrcImageStep[i], oSrcImageROI,
            apDstImage[i], aDstImageStep[i], oDstImageROI, nScaleFactor,
            nScaleFactor, apLanczosBuffer[i], NPPI_INTER_LANCZOS3_ADVANCED));
    }
    ...
    NPP_CHECK_NPP(nppiEncodeOptimizeHuffmanScan_JPEG_8u16s_P3R(
        ...
        hpCodesDC, hpTableDC, hpCodesAC, hpTableAC, apHuffmanDCTable,
        apHuffmanACTable, aDstSize, pJpegEncoderTemp));
