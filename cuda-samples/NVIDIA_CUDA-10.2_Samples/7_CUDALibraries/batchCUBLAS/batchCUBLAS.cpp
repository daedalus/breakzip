...
static inline cublasStatus_t cublasXgemmBatched(
    ...
    int batchCount) {
#if CUDART_VERSION >= 4010
    return cublasDgemmBatched(handle, transa, transb, m, n, k, alpha, Aarray,
                              lda, Barray, ldb, beta, Carray, ldc, batchCount);
#else
    return CUBLAS_STATUS_SUCCESS;
#endif
...
#define NBR_BETAS (sizeof(beta) / sizeof(beta[0]))
    ...
    printf("#### args: ta=%d tb=%d m=%d n=%d k=%d ",
           (unsigned int)params->transa, (unsigned int)params->transb,
...
int test_gemm_loop(struct gemmOpts &opts, float err, double max_relative_error,
    ...
    for (int i = 0; i < opts.N; i++) {
        cudaError_t err1 = cudaMalloc((void **)&devPtrA[i],
                                      matrixSizeA * sizeof(devPtrA[0][0]));
        cudaError_t err2 = cudaMalloc((void **)&devPtrB[i],
                                      matrixSizeB * sizeof(devPtrB[0][0]));
        cudaError_t err3 = cudaMalloc((void **)&devPtrC[i],
                                      matrixSizeC * sizeof(devPtrC[0][0]));
    ...
    while (TESTGEN(gemm)(&opts, matrixM, matrixN, matrixK, numTests, &params) ==
        ...
        if (opts.test_method == tmBatched) {
            ...
            status1 = cublasXgemmBatched(
                handle, params.transa, params.transb, params.m, params.n,
                params.k, &params.alpha, (const T_ELEM **)devPtrA_dev, rowsA,
                (const T_ELEM **)devPtrB_dev, rowsB, &params.beta, devPtrC_dev,
                rowsC, opts.N);
            ...
            if (status1 != CUBLAS_STATUS_SUCCESS) {
                ...
                CLEANUP();
                fprintf(stderr,
                        "!!!! GPU program execution error : cublas Error=%d, "
                        "cuda Error=%d,(%s)\n",
            ...
            for (int i = 0; i < opts.N; i++) {
                ...
                status1 = cublasXgemm(
                    handle, params.transa, params.transb, params.m, params.n,
                    params.k, &params.alpha, devPtrA[i], rowsA, devPtrB[i],
                    rowsB, &params.beta, devPtrC[i], rowsC);

                if (status1 != CUBLAS_STATUS_SUCCESS) {
                    cudaError_t cudaStatus = cudaGetLastError();
                    CLEANUP();
                    fprintf(stderr,
                            "!!!! GPU program execution error : cublas "
                            "Error=%d, cuda Error=%d,(%s)\n",
                            status1, cudaStatus,
...
int main(int argc, char *argv[]) {
    ...
    *(opts.elem_type) = 'd';
    ...
    // Run with and without streams and then batched. The batched functions are
    // a feature new feature in 4.1
#if CUDART_VERSION >= 4010
    ...
    for (int ii = 0; ii < 2; ii++) {
        ...
        *(opts.elem_type) = 's';
        status = test_gemm_loop<float>(opts, (float)CUBLAS_SGEMM_MAX_ULP_ERR,
                                       (double)CUBLAS_SGEMM_MAX_RELATIVE_ERR,
                                       handle);
        nTotalErrors += (status == CUBLASTEST_PASSED ? 0 : 1);
        ...
        *(opts.elem_type) = 'd';
        ...
        if (getDeviceVersion() < DEV_VER_DBL_SUPPORT) {
            fprintf(stdout,
                    "@@@@ dgemm test WAIVED due to lack of DP support\n");
            exit(EXIT_WAIVED);
        } else {
            status = test_gemm_loop<double>(
                opts, (float)CUBLAS_DGEMM_MAX_ULP_ERR,
                (double)CUBLAS_DGEMM_MAX_RELATIVE_ERR, handle);
            nTotalErrors += (status == CUBLASTEST_PASSED ? 0 : 1);
        }
    }
