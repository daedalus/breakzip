...
int main(int argc, char *argv[]) {
    ...
    try {
        ...
        if (infile.good()) {
            ...
            std::cout << "histEqualizationNPP unable to open: <"
        ...
        const int levelCount =
            binCount + 1;  // levels array has one more element
        ...
        NPP_CHECK_NPP(nppiHistogramEven_8u_C1R(
            oDeviceSrc.data(), oDeviceSrc.pitch(), oSizeROI, histDevice,
            levelCount, 0, binCount, pDeviceBuffer));
        // copy histogram and levels to host memory
        Npp32s histHost[binCount];
        NPP_CHECK_CUDA(cudaMemcpy(histHost, histDevice,
                                  binCount * sizeof(Npp32s),
        ...
        // fill LUT
        {
            ...
            for (pHostHistogram = histHost;
                 pHostHistogram < histHost + binCount; ++pHostHistogram) {
        ...
        // Note for CUDA 5.0, that nppiLUT_Linear_8u_C1R requires these pointers
        // to be in GPU device memory
        Npp32s *lutDevice = 0;
        Npp32s *lvlsDevice = 0;
        ...
        NPP_CHECK_CUDA(cudaMemcpy(lutDevice, lutHost,
