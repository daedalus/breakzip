...
int main(int argc, char *argv[]) {
    ...
    try {
        ...
        if (infile.good()) {
            ...
            std::cout << "freeImageInteropNPP unable to open: <"
        ...
        NPP_CHECK_CUDA(cudaMemcpy2D(pResultData, nResultPitch, pDstImageCUDA,
                                    nDstPitchCUDA, oSizeROI.width,
                                    oSizeROI.height, cudaMemcpyDeviceToHost));
        // now save the result image
        bool bSuccess;
        bSuccess = FreeImage_Save(FIF_PGM, pResultBitmap,
                                  sResultFilename.c_str(), 0) == TRUE;
