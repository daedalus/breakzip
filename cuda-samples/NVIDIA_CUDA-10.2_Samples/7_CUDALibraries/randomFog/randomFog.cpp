...
void reshape(int x, int y) {
    ...
    if (g_bTenXZoom) {
        glOrtho(-.15f * xScale, .15f * xScale, -.15f * yScale, .15f * yScale,
                -5.0f, 5.0f);
    } else {
...
void showHelp(void) {
    ...
    ss << "On creation, randomFog generates 200,000 random coordinates in "
       ...
       << "coordinate space (radius, angle rho, angle theta) with curand's "
          "XORWOW algorithm. "
       << "The coordinates are normalized for a uniform distribution through "
          "the sphere.\n\n";
    ss << "The X axis is drawn with blue in the negative direction and yellow "
          "positive.\n"
       << "The Y axis is drawn with green in the negative direction and "
          "magenta positive.\n"
    ...
    ss << "\t" << setw(10) << "p"
       << "Generate a new set of random numbers and display on a cartesian "
          "plane (Plane)\n\n";
    ss << "\t" << setw(10) << "i,l,j"
...
int main(int argc, char **argv) {
    ...
    try {
        ...
        if (checkCmdLineFlag(argc, (const char **)argv, "qatest")) {
            ...
            // Create a window with rendering context and everything else we
            // need
        ...
        if (bQA) {
            ...
            g_pCheckRender->dumpBin(g_pVertices,
                                    g_nVerticesPopulated * sizeof(SVertex),
                                    "randomFog.bin");
            ...
            // Enable the color array functionality (so we can specify a color
            // for each vertex)
