...
RNG::RNG(unsigned long prngSeed, unsigned int qrngDimensions,
    ...
    if (m_h_samples == NULL) {
        throw runtime_error(
            "Could not allocate host memory for RNG::m_h_samples");
    }

    // Allocate sample array in device mem
    cudaResult = cudaMalloc((void **)&m_d_samples,
                            m_nSamplesBatchTarget * sizeof(float));
    ...
    if (curandResult != CURAND_STATUS_SUCCESS) {
        string msg(
            "Could not create scrambled quasi-random number generator: ");
        msg += curandResult;
        throw runtime_error(msg);
    }
...
void RNG::generateBatch(void) {
    ...
    cudaResult = cudaMemcpy(m_h_samples, m_d_samples,
                            m_nSamplesBatchActual * sizeof(float),
...
void RNG::resetSeed(void) {
    ...
    if (curandResult != CURAND_STATUS_SUCCESS) {
        std::string msg(
            "Could not set pseudo-random number generator offset: ");
        msg += curandResult;
        throw runtime_error(msg);
    }
...
void RNG::updateDimensions(void) {
    ...
    if (curandResult != CURAND_STATUS_SUCCESS) {
        std::string msg(
            "Could not set quasi-random number generator dimensions: ");
        msg += curandResult;
        throw runtime_error(msg);
    }
    ...
    if (curandResult != CURAND_STATUS_SUCCESS) {
        std::string msg(
            "Could not set scrambled quasi-random number generator "
            "dimensions: ");
        msg += curandResult;
        throw runtime_error(msg);
    }
