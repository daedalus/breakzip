...
private:
    ...
    std::string
        sFileName_;  ///< Name of the file where the exception was thrown.
    unsigned int nLineNumber_;  ///< Line number in the file where the exception
                                ///< was thrown
};
...
#define NPP_DEBUG_ASSERT(C)                                              \
    do {                                                                 \
        if (!(C))                                                        \
            throw npp::Exception(#C " debug assertion faild!", __FILE__, \
                                 __LINE__);                              \
    } while (false)
#else
#define NPP_DEBUG_ASSERT(C)
