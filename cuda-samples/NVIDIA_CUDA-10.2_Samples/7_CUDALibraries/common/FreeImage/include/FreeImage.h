...
FI_STRUCT(FIICCPROFILE) {
    ...
    void *
        data;  //! points to a block of contiguous memory containing the profile
};
...
FI_ENUM(FREE_IMAGE_FORMAT){
    ...
    FIF_RAW = 34,     FIF_WEBP = 35,   FIF_JXR = 36};
...
FI_ENUM(FREE_IMAGE_TYPE){
    ...
    FIT_RGBAF =
        12  //! 128-bit RGBA float image	: 4 x 32-bit IEEE floating point
};
...
#define FIF_LOAD_NOPIXELS \
    0x8000  //! loading: load the image header only (not supported by all
            //! plugins, default to full loading)
...
#define EXR_B44 \
    0x0020  //! save with lossy 44% float compression - goes to 22% when
            //! combined with EXR_LC
#define EXR_LC \
    0x0040  //! save images with one luminance and two chroma channels, rather
...
#define JPEG_PROGRESSIVE \
    0x2000  //! save as a progressive-JPEG (use | to combine with other save
            //! flags)
...
#define JPEG_SUBSAMPLING_444 \
    0x10000  //! save with no chroma subsampling (4:4:4)
#define JPEG_OPTIMIZE \
    0x20000  //! on saving, compute optimal Huffman coding tables (can reduce a
...
#define RAW_PREVIEW \
    1  //! try to load the embedded JPEG preview with included Exif Data or
       //! default to RGB 24-bit
...
#define JXR_DEFAULT \
    0  //! save with quality 80 and no chroma subsampling (4:4:4)
#define JXR_LOSSLESS 0x0064  //! save lossless
#define JXR_PROGRESSIVE \
    0x2000  //! save as a progressive-JXR (use | to combine with other save
            //! flags)
...
#define FI_COLOR_ALPHA_IS_INDEX \
    0x04  //! The color's rgbReserved member (alpha) contains the palette index
          //! to be used
