...
int read_next_batch(FileNames &image_names, int batch_size,
    ...
    while (counter < batch_size) {
        if (cur_iter == image_names.end()) {
            std::cerr
                << "Image list is too short to fill the batch, adding files "
        ...
        if (image_names.size() == 0) {
            std::cerr << "No valid images left in the input list, exit"
                      << std::endl;
            return EXIT_FAILURE;
        }
...
int prepare_buffers(FileData &file_data, std::vector<size_t> &file_len,
    ...
    for (int i = 0; i < file_data.size(); i++) {
        checkCudaErrors(nvjpegGetImageInfo(
            params.nvjpeg_handle, (unsigned char *)file_data[i].data(),
        ...
        if (params.fmt == NVJPEG_OUTPUT_RGBI ||
    ...
    if (!params.batched) {
        if (!params.pipelined)  // decode one image at a time
        {
            ...
            for (int i = 0; i < params.batch_size; i++) {
                checkCudaErrors(nvjpegDecode(
                    params.nvjpeg_handle, params.nvjpeg_state,
                    (const unsigned char *)img_data[i].data(), img_len[i],
                    params.fmt, &out[i], params.stream));
                checkCudaErrors(cudaStreamSynchronize(params.stream));
            }
        } else {
            int thread_idx = 0;
            sdkStartTimer(&timer);
            for (int i = 0; i < params.batch_size; i++) {
                checkCudaErrors(nvjpegDecodePhaseOne(
                    params.nvjpeg_handle, params.nvjpeg_state,
                    (const unsigned char *)img_data[i].data(), img_len[i],
                    params.fmt, params.stream));
                checkCudaErrors(cudaStreamSynchronize(params.stream));
                checkCudaErrors(nvjpegDecodePhaseTwo(
                    params.nvjpeg_handle, params.nvjpeg_state, params.stream));
                checkCudaErrors(nvjpegDecodePhaseThree(params.nvjpeg_handle,
                                                       params.nvjpeg_state,
                                                       &out[i], params.stream));
            }
            checkCudaErrors(cudaStreamSynchronize(params.stream));
        ...
        if (!params.pipelined)  // decode multiple images in a single batch
        {
            ...
            checkCudaErrors(nvjpegDecodeBatchedPhaseTwo(
                params.nvjpeg_handle, params.nvjpeg_state, params.stream));
            checkCudaErrors(nvjpegDecodeBatchedPhaseThree(
                params.nvjpeg_handle, params.nvjpeg_state, out.data(),
                params.stream));
            checkCudaErrors(cudaStreamSynchronize(params.stream));
        }
    }
    ...
    for (int i = 0; i < params.batch_size; i++) {
        ...
        if (params.fmt == NVJPEG_OUTPUT_RGB ||
            ...
            err = writeBMP(fname.c_str(), iout[i].channel[0], iout[i].pitch[0],
                           ...
                           iout[i].channel[2], iout[i].pitch[2], widths[i],
                           heights[i]);
        ...
        std::cout << "Done writing decoded image to file: " << fname
                  << std::endl;
    }
}
...
double process_images(FileNames &image_names, decode_params_t &params,
    ...
    while (total_processed < params.total_images) {
        if (read_next_batch(image_names, params.batch_size, file_iter,
                            file_data, file_len, current_names))
            return EXIT_FAILURE;
...
int main(int argc, const char *argv[]) {
    ...
    if ((pidx = findParamIndex(argv, argc, "-h")) != -1 ||
        ...
        std::cout << "\tbatch_size\t:\tDecode images from input by batches of "
                  ...
                  << std::endl;
        std::cout
            << "\ttotal_images\t:\tDecode this much images, if there are "
               "less images \n"
        ...
        std::cout
            << "\toutput_format\t:\tnvJPEG output format for decoding. One "
    ...
    if ((pidx = findParamIndex(argv, argc, "-o")) != -1) {
        ...
        if (params.fmt != NVJPEG_OUTPUT_RGB &&
            ...
            params.fmt != NVJPEG_OUTPUT_BGRI) {
            std::cout
                << "We can write ony BMPs, which require output format be "
