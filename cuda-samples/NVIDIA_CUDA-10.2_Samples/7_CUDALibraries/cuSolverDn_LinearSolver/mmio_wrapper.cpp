...
static int verify_pattern(int m, int nnz, int *csrRowPtr, int *csrColInd) {
    ...
    if (nnz != (csrRowPtr[m] - csrRowPtr[0])) {
        fprintf(stderr,
                "Error (nnz check failed): (csrRowPtr[%d]=%d - "
                "csrRowPtr[%d]=%d) != (nnz=%d)\n",
    ...
    for (i = 0; (!error_found) && (i < m); i++) {
        ...
        if (start > end) {
            fprintf(stderr,
                    "Error (corrupted row): csrRowPtr[%d] (=%d) > "
                    "csrRowPtr[%d] (=%d)\n",
        ...
        for (col = start; col < end; col++) {
            if (csrColInd[col] < base_index) {
                fprintf(stderr,
                        "Error (column vs. base index check failed): "
                        "csrColInd[%d] < %d\n",
                        col, base_index);
                error_found = 1;
            }
...
int loadMMSparseMatrix(char *filename, char elem_type, bool csrFormat, int *m,
    ...
    if (mm_is_dense(matcode) || mm_is_array(matcode) ||
        ...
        fprintf(stderr,
                "!!!! dense, array, pattern and integer matrices are not "
                "supported\n");
        return 1;
    }
    ...
    if (csrFormat) {
        /* create row-major ordering of indices (sorted by row and within each
         * row by column) */
        qsort(work, *nnz, sizeof(struct cooFormat), (FUNPTR)fptr_array[0]);
    } else {
        /* create column-major ordering of indices (sorted by column and within
         * each column by row) */
    ...
    /* transfrom the matrix values of type double into one of the cusparse
     * library types */
    *aVal = (T_ELEM *)malloc((*nnz) * sizeof(T_ELEM));
    ...
    for (i = 0; i < (*nnz); i++) {
        ...
        if (mm_is_real(matcode) || mm_is_integer(matcode)) {
            ...
            (*aVal)[i] = cuGet<T_ELEM>(tempVal[2 * work[i].p],
                                       tempVal[2 * work[i].p + 1]);
        }
    }
