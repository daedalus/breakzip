...
int linearSolverCHOL(cusolverDnHandle_t handle, int n, const double *Acopy,
    ...
    checkCudaErrors(cusolverDnDpotrf_bufferSize(
        handle, uplo, n, (double *)Acopy, lda, &bufferSize));
    ...
    checkCudaErrors(cudaMemcpy(A, Acopy, sizeof(double) * lda * n,
...
int linearSolverQR(cusolverDnHandle_t handle, int n, const double *Acopy,
    ...
    // compute Q^T*b
    checkCudaErrors(cusolverDnDormqr(handle, CUBLAS_SIDE_LEFT, CUBLAS_OP_T, n,
                                     1, n, A, lda, tau, x, n, buffer,
                                     bufferSize, info));
...
int main(int argc, char *argv[]) {
    ...
    if (opts.sparse_mat_filename == NULL) {
        opts.sparse_mat_filename =
            sdkFindFilePath("gr_900_900_crg.mtx", argv[0]);
        if (opts.sparse_mat_filename != NULL)
    ...
    if (lda < rowsA) {
        fprintf(stderr,
                "Error: lda must be greater or equal to dimension of A\n");
        exit(EXIT_FAILURE);
    }
    ...
    checkCudaErrors(cublasDgemm_v2(cublasHandle, CUBLAS_OP_N, CUBLAS_OP_N,
                                   rowsA, 1, colsA, &minus_one, d_A, lda, d_x,
                                   rowsA, &one, d_r, rowsA));
