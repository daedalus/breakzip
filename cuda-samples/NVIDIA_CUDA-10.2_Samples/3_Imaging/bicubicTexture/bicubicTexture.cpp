...
void display() {
    ...
    // Common display path
    {
        ...
        glProgramLocalParameterI4iNV(GL_FRAGMENT_PROGRAM_ARB, 0, width, 0, 0,
                                     0);
#else
...
void runBenchmark(int iterations) {
    ...
    for (int i = 0; i < iterations; ++i) {
        render(imageWidth, imageHeight, tx, ty, scale, cx, cy, blockSize,
               gridSize, g_FilterMode, d_output);
    }
...
void runAutoTest(int argc, char **argv, const char *dump_filename,
    ...
    uchar4 *d_output;
    checkCudaErrors(
        cudaMalloc((void **)&d_output, imageWidth * imageHeight * 4));
...
void loadImageData(int argc, char **argv) {
    ...
    if ((srcImagePath = sdkFindFilePath(srcImageFilename, argv[0])) == NULL) {
        printf(
            "bicubicTexture loadImageData() could not find <%s>\nExiting...\n",
            srcImageFilename);
        exit(EXIT_FAILURE);
    }
...
int main(int argc, char **argv) {
    ...
    if (checkCmdLineFlag(argc, (const char **)argv, "mode")) {
        g_FilterMode = (eFilterMode)getCmdLineArgumentInt(
            argc, (const char **)argv, "mode");
    ...
    if (getCmdLineArgumentString(argc, (const char **)argv, "file",
                                 &filename)) {
