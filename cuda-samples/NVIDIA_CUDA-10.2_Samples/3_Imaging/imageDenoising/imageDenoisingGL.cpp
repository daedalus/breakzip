...
void runImageFilters(TColor *d_dst) {
    switch (g_Kernel) {
        ...
        case 1:
            if (!g_Diag) {
                cuda_KNN(d_dst, imageW, imageH, 1.0f / (knnNoise * knnNoise),
                         lerpC, texImage);
            } else {
                cuda_KNNdiag(d_dst, imageW, imageH,
                             1.0f / (knnNoise * knnNoise), lerpC, texImage);
            }
        ...
        case 2:
            if (!g_Diag) {
                cuda_NLM(d_dst, imageW, imageH, 1.0f / (nlmNoise * nlmNoise),
                         lerpC, texImage);
            } else {
                cuda_NLMdiag(d_dst, imageW, imageH,
                             1.0f / (nlmNoise * nlmNoise), lerpC, texImage);
            }
        ...
        case 3:
            if (!g_Diag) {
                cuda_NLM2(d_dst, imageW, imageH, 1.0f / (nlmNoise * nlmNoise),
                          lerpC, texImage);
            } else {
                cuda_NLM2diag(d_dst, imageW, imageH,
                              1.0f / (nlmNoise * nlmNoise), lerpC, texImage);
            }
...
void initOpenGLBuffers() {
    ...
    if (gl_error != GL_NO_ERROR) {
        ...
        sprintf_s(tmpStr, 255, "\n%s(%i) : GL Error : %s\n\n", __FILE__,
                  __LINE__, gluErrorString(gl_error));
...
int main(int argc, char **argv) {
    ...
    if (checkCmdLineFlag(argc, (const char **)argv, "file")) {
        ...
        if (checkCmdLineFlag(argc, (const char **)argv, "device")) {
            ...
            printf(
                "   To use -device=n, the sample must be running w/o "
                "OpenGL\n\n");
        ...
        // First load the image, so we know what the size of the image (imageW
        // and imageH)
        printf("Allocating host and CUDA memory and loading image file...\n");
        const char *image_path = sdkFindFilePath("portrait_noise.bmp", argv[0]);
