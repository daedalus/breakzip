...
static void printHelp(const char *app_name) {
    ...
    std::cout << "\t-width=width                input nv12 image width, <1 -- "
                 "4096>\n";
    std::cout << "\t-height=height              input nv12 image height, <1 -- "
                 "4096>\n";
    std::cout << "\t-pitch=pitch(optional)      input nv12 image pitch, <0 -- "
                 "4096>\n";
    std::cout << "\t-dst_width=width            output BGR image width, <1 -- "
                 "4096>\n";
    std::cout << "\t-dst_height=height          output BGR image height, <1 -- "
                 "4096>\n";
    std::cout << "\t-dst_pitch=pitch(optional)  output BGR image pitch, <0 -- "
                 "4096>\n";
    std::cout << "\t-batch=batch                process frames count, <1 -- "
                 "4096>\n\n";
    std::cout << "\t-device=device_num(optional)   cuda device number, <0 -- "
                 "4096>\n\n";
...
int parseCmdLine(int argc, char *argv[]) {
    ...
    if (argc == 1) {
        ...
        if (checkCmdLineFlag(argc, (const char **)argv, "width")) {
            g_ctx.width =
                getCmdLineArgumentInt(argc, (const char **)argv, "width");
        }
        ...
        if (checkCmdLineFlag(argc, (const char **)argv, "pitch")) {
            g_ctx.pitch =
                getCmdLineArgumentInt(argc, (const char **)argv, "pitch");
        }
        ...
        if (checkCmdLineFlag(argc, (const char **)argv, "batch")) {
            g_ctx.batch =
                getCmdLineArgumentInt(argc, (const char **)argv, "batch");
        }
    }
...
static int loadNV12Frame(unsigned char *d_inputNV12) {
    ...
    int frameSize;
    std::ifstream nv12File(g_ctx.input_nv12_file,
    ...
    for (int i = 0; i < g_ctx.batch; i++) {
        checkCudaErrors(cudaMemcpy2D(
            (void *)d_nv12, g_ctx.ctx_pitch, pNV12FrameData, g_ctx.width,
            g_ctx.width, g_ctx.ctx_heights, cudaMemcpyHostToDevice));
...
void nv12ResizeAndNV12ToBGR(unsigned char *d_inputNV12) {
    ...
    size = g_ctx.dst_width * ceil(g_ctx.dst_height * 3.0f / 2.0f) *
           g_ctx.batch * sizeof(unsigned char);
    ...
    printf(
        ...
        g_ctx.width, g_ctx.height, g_ctx.dst_width, g_ctx.dst_height,
...
#if USE_UVM_MEM
    checkCudaErrors(
        cudaMallocManaged((void **)&d_inputNV12,
