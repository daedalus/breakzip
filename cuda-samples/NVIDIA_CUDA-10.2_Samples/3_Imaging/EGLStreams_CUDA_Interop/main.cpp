    ...
    for (i = 0; i < NUM_TRAILS; i++) {
        ...
        for (j = 0; j < 2; j++) {
            ...
            if (j == 0) {
                checkCudaErrors(cuCtxPushCurrent(cudaProducer.context));
                curesult =
                    cudaProducerTest(&cudaProducer, cudaProducer.fileName1);
                if (curesult != CUDA_SUCCESS) {
                ...
                checkCudaErrors(cuCtxPushCurrent(cudaConsumer.context));
                curesult =
                    cudaConsumerTest(&cudaConsumer, cudaConsumer.outFile1);
                if (curesult != CUDA_SUCCESS) {
                ...
                checkCudaErrors(cuCtxPushCurrent(cudaProducer.context));
                curesult =
                    cudaProducerTest(&cudaProducer, cudaProducer.fileName2);
                if (curesult != CUDA_SUCCESS) {
                    printf("Cuda Producer Test failed for frame = %d\n", j + 1);
                    goto done;
                }
                ...
                checkCudaErrors(cuCtxPushCurrent(cudaConsumer.context));
                curesult =
                    cudaConsumerTest(&cudaConsumer, cudaConsumer.outFile2);
                if (curesult != CUDA_SUCCESS) {
                    printf("Cuda Consumer Test failed for frame = %d\n", j + 1);
                    goto done;
                }
