...
void initMC(int argc, char **argv) {
    ...
    gridSize = make_uint3(1 << gridSizeLog2.x, 1 << gridSizeLog2.y,
                          1 << gridSizeLog2.z);
    ...
    if (g_bValidate) {
        ...
        checkCudaErrors(
            cudaGraphicsGLRegisterBuffer(&cuda_normalvbo_resource, normalVbo,
...
void computeIsosurface() {
    ...
    launch_generateTriangles2(grid2, NTHREADS, d_pos, d_normal,
                              d_compVoxelArray, d_voxelVertsScan, d_volume,
                              gridSize, gridSizeShift, gridSizeMask, voxelSize,
                              isoValue, activeVoxels, maxVerts);
#else
