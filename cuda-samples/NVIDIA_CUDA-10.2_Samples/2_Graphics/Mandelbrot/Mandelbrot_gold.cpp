...
void runMandelbrotGold0(uchar4 *dst, const int imageW, const int imageH,
    ...
    for (int iy = 0; iy < imageH; iy++)
        for (int ix = 0; ix < imageW; ix++) {
            ...
            int m = CalcMandelbrot<T_>(xPos, yPos, xJParam, yJParam, crunch,
                                       isJulia);
            m = m > 0 ? crunch - m : 0;
...
inline dfloat operator*(const dfloat &dsa, const dfloat &dsb) {
    ...
    // Compute dsa[0] * dsb[1] + dsa[1] * dsb[0] (only high-order word is
    // needed).
    float c2 = dsa[0] * dsb[1] + dsa[1] * dsb[0];

    // Compute (c11, c21) + c2 using Knuth's trick, also adding low-order
    // product.
    float t1 = c11 + c2;
    float e = t1 - c11;
