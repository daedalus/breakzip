...
void renderImage(bool bUseOpenGL, bool fp64, int mode) {
    ...
    if (pass < 128) {
        if (g_runCPU) {
            ...
            if (bUseOpenGL) {
                // DEPRECATED:
                // checkCudaErrors(cudaGLMapBufferObject((void**)&d_dst,
                // gl_PBO));
                checkCudaErrors(
            ...
            if (pass && !startPass)  // Use the adaptive sampling version when
                                     // animating.
            {
                if (precisionMode)
                    RunMandelbrotDSGold1(h_Src, imageW, imageH, crunch, x, y,
                                         xJParam, yJParam, s, colors, pass++,
                                         animationFrame, g_isJuliaSet);
                else
                    RunMandelbrotGold1(h_Src, imageW, imageH, crunch, (float)x,
                                       (float)y, (float)xJParam, (float)yJParam,
                                       (float)s, colors, pass++, animationFrame,
                                       g_isJuliaSet);
            } else {
                if (precisionMode)
                    ...
                    RunMandelbrotGold0(h_Src, imageW, imageH, crunch, (float)x,
                                       (float)y, (float)xJParam, (float)yJParam,
                                       (float)s, colors, pass++, animationFrame,
                                       g_isJuliaSet);
            }

            checkCudaErrors(cudaMemcpy(d_dst, h_Src,
        ...
        } else  // this is the GPU Path
        {
            ...
            // Render anti-aliasing passes until we run out time (60fps
            // approximately)
            do {
                float xs, ys;
                ...
                // Run the mandelbrot generator
                if (pass && !startPass)  // Use the adaptive sampling version
                                         // when animating.
                    RunMandelbrot1(d_dst, imageW, imageH, crunch, x, y, xJParam,
                                   yJParam, s, colors, pass++, animationFrame,
                                   precisionMode, numSMs, g_isJuliaSet,
                                   version);
                else
                    RunMandelbrot0(d_dst, imageW, imageH, crunch, x, y, xJParam,
                                   yJParam, s, colors, pass++, animationFrame,
                                   precisionMode, numSMs, g_isJuliaSet,
                                   version);

                // Estimate the total time of the frame if one more pass is
                // rendered
                timeEstimate =
...
void keyboardFunc(unsigned char k, int, int) {
    ...
    switch (k) {
        ...
        case 'd':
            if (2 * crunch <=
                MIN(numSMs * (version < 20 ? 512 : 2048), 0x4000)) {
                crunch *= 2;
                pass = 0;
            }
        ...
        case 'O':
            ...
            fseek(stream, 0L, SEEK_SET);
            fscanf(stream, "%lf %lf %lf %lf %lf", &xOff, &yOff, &scale,
                   &xJParam, &yJParam);
            xdOff = 0.0;
            ydOff = 0.0;
            dscale = 1.0;
...
int runSingleTest(int argc, char **argv) {
    ...
    // We've already determined that file has been passed in as input, we can
    // grab the file here
    ...
    if (g_isJuliaSet) {
        ...
        checkCudaErrors(cudaMemcpy(h_dst, d_dst,
                                   imageW * imageH * sizeof(uchar4),
