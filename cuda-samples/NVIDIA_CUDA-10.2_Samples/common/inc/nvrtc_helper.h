...
void compileFileToPTX(char *filename, int argc, char **argv, char **ptxResult,
    ...
    if (requiresCGheaders) {
        ...
        sprintf_s(HeaderNames, sizeof(HeaderNames), "%s",
                  "cooperative_groups.h");
#else
        snprintf(HeaderNames, sizeof(HeaderNames), "%s",
                 "cooperative_groups.h");
#endif
        ...
        if (!path.empty()) {
            ...
            printf(
                "\nCooperativeGroups headers not found, please install it in "
                "%s "
                "sample directory..\n Exiting..\n",
                argv[0]);
        }
...
#if defined(WIN32) || defined(_WIN32) || defined(WIN64) || defined(_WIN64)
        sprintf_s(compileParams[0],
                  sizeof(char) * (compileOptions.length() + 1), "%s",
    ...
    nvrtcResult res =
        nvrtcCompileProgram(prog, numCompileOptions, compileParams);
