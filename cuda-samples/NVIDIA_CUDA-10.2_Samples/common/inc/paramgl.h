...
public:
    ...
    void Render(int x, int y, bool shadow = false) {
        ...
        for (std::vector<ParamBase *>::const_iterator p = m_params.begin();
            ...
            if ((*p)->IsList()) {
                ...
                if (shadow) {
                    glPrintShadowed(x + m_text_x, y + m_font_h,
                                    (*p)->GetName().c_str(), m_font,
                                    (p == m_current)
                                        ? &m_text_color_selected.r
                    ...
                    glPrint(x + m_text_x, y + m_font_h, (*p)->GetName().c_str(),
                            m_font);
                    glPrint(x + m_value_x, y + m_font_h,
                            (*p)->GetValueString().c_str(), m_font);
                }
    ...
    bool Mouse(int x, int y, int button = GLUT_LEFT_BUTTON,
        ...
        if ((button == GLUT_LEFT_BUTTON) && (state == GLUT_DOWN)) {
            ...
            // non-standard
            for (m_current = m_params.begin();
                 m_current != m_params.end() && i > 0; m_current++, i--)
                ;
