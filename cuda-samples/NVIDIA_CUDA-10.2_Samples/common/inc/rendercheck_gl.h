...
public:
    ...
    virtual void savePGM(const char *zfilename, bool bInvert,
                         void **ppReadBuf) {
        if (zfilename != NULL) {
            if (bInvert) {
                unsigned char *readBuf;
                unsigned char *writeBuf =
                    (unsigned char *)malloc(m_Width * m_Height);
            ...
            if (ppReadBuf) {
                sdkSavePGM<unsigned char>(
                    zfilename, *(unsigned char **)ppReadBuf, m_Width, m_Height);
            } else {
                sdkSavePGM<unsigned char>(zfilename,
    ...
    virtual void savePPM(const char *zfilename, bool bInvert,
                         void **ppReadBuf) {
        if (zfilename != NULL) {
            if (bInvert) {
                unsigned char *readBuf;
    ...
    virtual bool PGMvsPGM(const char *src_file, const char *ref_file,
        ...
        if (ref_file_path == NULL) {
            ...
            if (sdkLoadPPMub(ref_file_path, &ref_data, &width, &height) !=
                true) {
            ...
            if (compareDataAsFloatThreshold<unsigned char, float>(
                    ref_data, src_data, m_Height * m_Width, epsilon,
                    threshold) == false) {
                error_count = 1;
            }
        }
    ...
    virtual bool PPMvsPPM(const char *src_file, const char *ref_file,
        ...
        printf("   src_file <%s>\n", src_file);
        printf("   ref_file <%s>\n", ref_file_path);
        return (sdkComparePPM(src_file, ref_file_path, epsilon, threshold,
                              true) == true
                    ? true
                    : false);
    }
    ...
    virtual bool compareBin2BinUint(const char *src_file, const char *ref_file,
        ...
        if (src_fp == NULL) {
            printf(
                "compareBin2Bin <unsigned int> unable to open src_file: %s\n",
                src_file);
            error_count++;
        }
        ...
        if (ref_file_path == NULL) {
            printf(
                "compareBin2Bin <unsigned int>  unable to find <%s> in <%s>\n",
                ref_file, m_ExecPath.c_str());
            printf(">>> Check info.xml and [project//data] folder <%s> <<<\n",
                   ref_file);
            printf("Aborting comparison!\n");
            ...
            if (ref_fp == NULL) {
                printf(
                    "compareBin2Bin <unsigned int>  unable to open ref_file: "
                    "%s\n",
                    ref_file_path);
                error_count++;
            }
            ...
            if (src_fp && ref_fp) {
                ...
                fsize =
                    fread(src_buffer, sizeof(unsigned int), nelements, src_fp);

                if (fsize != nelements) {
                    printf(
                        "compareBin2Bin <unsigned int>  failed to read %u "
                        "elements from %s\n",
                        nelements, src_file);
                    error_count++;
                }
                ...
                fsize =
                    fread(ref_buffer, sizeof(unsigned int), nelements, ref_fp);
                ...
                if (!compareData<unsigned int, float>(ref_buffer, src_buffer,
                                                      nelements, epsilon,
                                                      threshold)) {
                    error_count++;
                }
    ...
    virtual bool compareBin2BinFloat(const char *src_file, const char *ref_file,
        ...
        if (ref_file_path == NULL) {
            ...
            if (src_fp && ref_fp) {
                ...
                if (fsize != nelements) {
                    printf(
                        "compareBin2Bin <float>  failed to read %u elements "
                        "from %s\n",
                        nelements, src_file);
                    error_count++;
                }
                ...
                if (!compareDataAsFloatThreshold<float, float>(
                        ref_buffer, src_buffer, nelements, epsilon,
                        threshold)) {
                    error_count++;
                }
...
public:
    ...
    virtual bool readback(GLuint width, GLuint height) {
        ...
        if (m_bUsePBO) {
            ...
            glReadPixels(0, 0, width, height, getPixelFormat(),
    ...
    virtual bool readback(GLuint width, GLuint height, GLuint bufObject) {
        ...
        if (m_bUseFBO) {
            if (m_bUsePBO) {
                ...
                if (!ret) {
                    printf(
                        "CheckBackBuffer::readback() FBO->PBO checkStatus = "
                        "%d\n",
                        ret);
                }
                ...
                if (!ret) {
                    printf(
                        "CheckBackBuffer::readback::glBindFramebufferEXT() "
                        "fbo=%d checkStatus = %d\n",
                        bufObject, ret);
                }
                ...
                if (!ret) {
                    printf(
                        "CheckBackBuffer::readback::glReadBuffer() fbo=%d "
                        "checkStatus = %d\n",
                        bufObject, ret);
                }
            ...
            unsigned char *temp_buf =
                (unsigned char *)malloc(width * height * m_Bpp);
...
public:
    CFrameBufferObject(unsigned int width, unsigned int height,
        ...
        m_fboData.depthTex =
            createTexture(m_eGLTarget, width, height, GL_DEPTH_COMPONENT24,
                          GL_DEPTH_COMPONENT);

        attachTexture(m_eGLTarget, m_fboData.colorTex,
        ...
        switch (texTarget) {
            ...
            case GL_TEXTURE_3D:
                glFramebufferTexture3DEXT(GL_FRAMEBUFFER_EXT, attachment,
                                          GL_TEXTURE_3D, texId, mipLevel,
                                          zSlice);
                break;

            default:
                // Default is GL_TEXTURE_2D, GL_TEXTURE_RECTANGLE_ARB, or cube
                // faces
    ...
    bool initialize(unsigned width, unsigned height, fboConfig &rConfigFBO,
        ...
        // If the FBO can be created, add it to the list of available configs,
        // and make a menu entry
        ...
        if (m_bUseFloat) {
            ...
            m_textureProgram = nv::CompileASMShader(GL_FRAGMENT_PROGRAM_ARB,
                                                    strTextureProgram2D);
            ...
            m_overlayProgram = nv::CompileASMShader(GL_FRAGMENT_PROGRAM_ARB,
                                                    strOverlayProgram);
        }
    ...
    bool create(GLuint width, GLuint height, fboConfig &config, fboData &data) {
        ...
        glTexImage2D(m_eGLTarget, 0, config.colorFormat, width, height, 0,
                     GL_RGBA, (m_bUseFloat ? GL_FLOAT : GL_UNSIGNED_BYTE),
                     NULL);
...

        {
            ...
            glFramebufferTexture2DEXT(GL_FRAMEBUFFER_EXT,
                                      GL_COLOR_ATTACHMENT0_EXT, m_eGLTarget,
                                      data.colorTex, 0);
            ...
            glFramebufferTexture2DEXT(GL_FRAMEBUFFER_EXT,
                                      GL_DEPTH_ATTACHMENT_EXT, m_eGLTarget,
                                      data.depthTex, 0);
...
public:
    ...
    virtual bool readback(GLuint width, GLuint height) {
        ...
        if (m_bUsePBO) {
            ...
            if (!ret) {
                printf(
                    "CheckFBO::readback::glBindFramebufferEXT() checkStatus = "
                    "%d\n",
                    ret);
            }
    ...
    virtual bool readback(GLuint width, GLuint height, GLuint bufObject) {
        ...
        if (m_bUseFBO) {
            if (m_bUsePBO) {
                ...
                if (!ret) {
                    printf(
                        "CheckFBO::readback::glReadBuffer() fbo=%d checkStatus "
                        "= %d\n",
                        (int)bufObject, (int)ret);
                }
