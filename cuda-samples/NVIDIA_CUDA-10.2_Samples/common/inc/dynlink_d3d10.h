...
struct DXUTD3D10DeviceSettings {
    ...
    UINT32 SyncInterval;
    DWORD PresentFlags;
    bool AutoCreateDepthStencil;  // DXUT will create the a depth stencil
                                  // resource and view if true
...
struct DXUTDeviceSettings {
    ...
    union {
        ...
        DXUTD3D10DeviceSettings
            d3d10;  // only valid if ver == DXUT_D3D10_DEVICE
    };
};
...
static bool dynlinkLoadD3D10API(void) {
    ...
    if (g_hModD3D10 != NULL) {
        ...
        sFnPtr_D3D10CreateDevice = (LPD3D10CREATEDEVICE)GetProcAddress(
            g_hModD3D10, "D3D10CreateDevice");

        sFnPtr_D3D10StateBlockMaskUnion =
            (LPD3D10STATEBLOCKMASKUNION)GetProcAddress(
                g_hModD3D10, "D3D10StateBlockMaskUnion");
