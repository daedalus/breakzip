...
void printHelp(int argc, char *argv[]) {
    ...
    std::cout
        << "\t                Each vector will consist of N components\n\n";
    std::cout << std::endl;
}
...
int main(int argc, char *argv[]) {
    ...
    if (checkCmdLineFlag(argc, (const char **)argv, "qatest")) {
        ...
        if (checkCmdLineFlag(argc, (const char **)argv, "vectors")) {
            n_vectors =
                getCmdLineArgumentInt(argc, (const char **)argv, "vectors");

            if (n_vectors < 1) {
                std::cerr << "Illegal argument: number of vectors must be "
                             "positive (--vectors=N)"
                          << std::endl;
                ok = false;
            }
        ...
        if (checkCmdLineFlag(argc, (const char **)argv, "dimensions")) {
            ...
            if (n_dimensions < 1) {
                std::cerr << "Illegal argument: number of dimensions must be "
                             "positive (--dimensions=N)"
                          << std::endl;
                ok = false;
            }
        }
    ...
    if (time < 1e-6) {
        ...
        std::cout << "Gsamples/s: "
                  << (double)n_vectors * (double)n_dimensions * 1E-9 /
                         (time * 1E-3)
                  << std::endl;
    }
    ...
    if (n_vectors == 1) {
        ...
        if (l1norm_ref != 0) {
            std::cerr << "Error: L1-Norm of the reference is not zero (for "
                         "single vector), golden generator appears broken\n";
        } else {
        ...
        if (l1norm_ref == 0) {
            std::cerr << "Error: L1-Norm of the reference is zero, golden "
                         "generator appears broken\n";
        } else {
            std::cout << "L1-Error: " << l1error << std::endl;
        }
    }
