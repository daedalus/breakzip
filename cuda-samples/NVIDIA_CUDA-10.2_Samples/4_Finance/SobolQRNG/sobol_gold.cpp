    ...
    for (int dim = 0; dim < n_dimensions; dim++) {
        ...
        if (dim == 0) {
            ...
            for (int i = d; i < n_directions; i++) {
                ...
                // compactness in the input tables they are stored as bits of a
                // single integer. To extract the relevant bit we use right
                // shift and mask with 1. For example, for a 10 degree
                // polynomial there are ten useful bits in a, so to get a[2] we
                // need to right shift 7 times (to get the 8th bit into the LSB)
                // and then mask with 1.
                for (int j = 1; j < d; j++) {
