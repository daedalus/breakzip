...
static void multiSolver(TOptionPlan *plan, int nPlans) {
    ...
    cudaStream_t *streams =
        (cudaStream_t *)malloc(nPlans * sizeof(cudaStream_t));
    ...
    // Init Each GPU
    // In CUDA 4.0 we can call cudaSetDevice multiple times to target each
    // device Set the device desired, then perform initializations on that
    // device
