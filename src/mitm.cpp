        ...
        for (uint16_t chunk2 = 0; chunk2 < 0x100; ++chunk2) {
            for (uint16_t chunk3 = 0; chunk3 < 0x100; ++chunk3) {
                for (uint8_t carries = 0; carries < 0x10; ++carries) {
                    if (s0 == c.sx[0][0] && chunk2 == c.chunk2 &&
                        chunk3 == c.chunk3 && carries == (c.carries >> 12)) {
    ...
    for (uint16_t s1xf0 = 0; s1xf0 < 0x100; ++s1xf0) {
        for (uint8_t prefix = 0; prefix < 0x40; ++prefix) {
            ...
            for (uint16_t s1xf1 = 0; s1xf1 < 0x100; ++s1xf1) {
                ...
                uint8_t s1yf1 =
                    s1xf1 ^ test_bytes[1][0][1] ^ test_bytes[1][2][1];
                        ...
                        for (auto t : thirds) {
                            ...
                            for (stage1a candidate : table1[mapkey]) {
                                ...
                                g.crck20 = ((pxf0 << 2) ^
                                           ...
                                           0xffff;
                                // Can iterate thru 64 preimages of s0 * 2 low
                                // bits to find a:b:low s.t. 7..2 of (a ^
                                // crc32tab[b:low]) = 7..2 of crck20; get 4
                                // solutions for msb crck20 Don't need to do it
                                // here.
                                g.k11msbs =
                                    mapkey ^ (candidate.msbk11xf0 * 0x01010101);
                                g.k21xf0 = pxf0;
...
void stage2() {
    ...
    for (uint8_t sample = 0; sample < 20; ++sample) {
            ...
            for (uint16_t chunk7 = 0; chunk7 < 0x100; ++chunk7) {
                for (uint8_t cb2 = 0; cb2 < 0x10; ++cb2) {
                    if (chunk6 == c.chunk6 && chunk7 == c.chunk7 &&
                        cb2 == ((c.carries >> 8) & 0xf)) {
                        fprintf(stderr,
                                "Stage 2a, should be on correct guess.\n");
                    }
                    ...
                    uint32_t msbxf0 = first_half_step(
                        test_bytes[0][0][0], false, c1.chunk3, carryx0f0, k0crc,
                        extra, upper1, lower1);
                    msbxf0 = first_half_step(test_bytes[0][0][1], true, chunk7,
                                             carryx1f0, k0crc, extra, upper2,
                                             lower2);
                    k0crc = c1.chunk2 | (chunk6 << 8);
                    extra = 0;
                    uint32_t msbyf0 = first_half_step(
                        test_bytes[0][0][0] ^ c1.s0, false, c1.chunk3,
                        carryy0f0, k0crc, extra, upper1, lower1);
                    msbyf0 = first_half_step(test_bytes[0][0][1] ^ c1.s1xf0,
                                             true, chunk7, carryy1f0, k0crc,
                                             extra, upper2, lower2);
                    if (upper1 < lower1) {
                    ...
                    k0crc = c1.chunk2 | (chunk6 << 8);
                    extra = 0;
                    uint32_t msbxf1 = first_half_step(
                        test_bytes[1][0][0], false, c1.chunk3, carryx0f1, k0crc,
                        extra, upper1, lower1);
                    msbxf1 = first_half_step(test_bytes[1][0][1], true, chunk7,
                                             carryx1f1, k0crc, extra, upper2,
                                             lower2);
                    if (upper1 < lower1) {
                        fprintf(stderr, "ERROR: Should never happen!\n");
                        abort();
                    ...
                    k0crc = c1.chunk2 | (chunk6 << 8);
                    extra = 0;
                    uint32_t msbyf1 = first_half_step(
                        test_bytes[1][0][0] ^ c1.s0, false, c1.chunk3,
                        carryy0f1, k0crc, extra, upper1, lower1);
                    msbyf1 = first_half_step(test_bytes[1][0][1] ^ c1.s1xf1,
                                             true, chunk7, carryy1f1, k0crc,
                                             extra, upper2, lower2);
                    if (upper1 < lower1) {
                        fprintf(stderr, "ERROR: Should never happen!\n");
                        abort();
        ...
        Continuing to expand, the result is the crc of a constant that depends
        on the stage1 candidate with a value from stage 2. =
        ...
        */
        ...
        for (uint16_t s2xf0 = 0; s2xf0 < 0x100; ++s2xf0) {
            ...
            for (uint8_t prefix = 0; prefix < 0x40; ++prefix) {
                ...
                for (uint16_t s2xf1 = 0; s2xf1 < 0x100; ++s2xf1) {
                    ...
                    vector<uint8_t> thirds(0);
                    uint8_t s2yf1 =
                        s2xf1 ^ test_bytes[1][0][2] ^ test_bytes[1][2][2];
                    ...
                    for (auto f : firsts) {
                        for (auto s : seconds) {
                            for (auto t : thirds) {
                                // xoring with these c?f? bytes as in the last
                                // line of the equation above
                                uint32_t mapkey((f ^ cyf0l) |
                                                ((s ^ cxf1l) << 8) |
                                                ((t ^ cyf1l) << 16));
                                for (auto c2 : table2[mapkey]) {
                                    // TODO: reconstruct k20. Can we propagate
                                    // the lowbits here to lowbits of k20? We
                                    // have s0, s1xf0, s1xf1, s2xf0, s2xf1 as
                                    // constraints on k20. Possibly
                                    // overspecified, may filter more bits.  If
                                    // not, at least we have k20 and maybe can
                                    // do a MITM on 8:9,cb in stage 3.
                                    bool viable = false;
                                    for (uint8_t lowbits = 0; lowbits < 4;
                                         ++lowbits) {
                                        if ((pxf0 & 0x3f) ==
                                            ((crc32((c1.k21xf0 << 2) | lowbits,
                                                    c2.msbk12xf0) >>
                                              2) &
                                             0x3f)) {
                                            viable = true;
                                    ...
                                    g.cb2 = c2.cb;
                                    g.k12msbs =
                                        mapkey ^ (c2.msbk12xf0 * 0x01010101);
                                    g.s2xf0 = s2xf0;
                                    g.prefix = prefix;
                                    g.s2xf1 = s2xf1;
                                    g.k22xf0 = pxf0;
                                    ...
                                    if (g.g1.chunk2 == c.chunk2 &&
                                        g.g1.chunk3 == c.chunk3 &&
                                        g.g1.cb == (c.carries >> 12) &&
                                        g.chunk6 == c.chunk6 &&
                                        g.chunk7 == c.chunk7 &&
        ...
        fprintf(stderr,
                "total_mapkeys = %04x, s2candidates.size() == %04lx\n\n",
