...
void mitm_stage2a(archive_info& info, stage1_candidate& c1, vector<vector<stage2a>>& table, correct_guess *c) {
  uint8_t cb1 = c1.cb1;
  ...
  for (uint16_t chunk6 = 0; chunk6 < 0x100; ++chunk6) {
    for (uint16_t chunk7 = 0; chunk7 < 0x100; ++chunk7) {
      for (uint8_t cb2 = 0; cb2 < 0x10; ++cb2) {
        ...
        msbxf0 = first_half_step(info.file[0].x[1], true, chunk7, carryx1f0,
        ...
        msbyf0 = first_half_step(info.file[0].x[1] ^ s1xf0, true, chunk7,
        ...
        uint32_t msbxf1 =
            first_half_step(info.file[1].x[0], false, c1.chunk3, carryx0f1,
                            k0crc, extra, upper1, lower1);
        msbxf1 = first_half_step(info.file[1].x[1], true, chunk7, carryx1f1,
                                 k0crc, extra, upper2, lower2);
        if (upper1 < lower1) {
          fprintf(stderr, "ERROR: Should never happen!\n");
        }
        ...
        msbyf1 = first_half_step(info.file[1].x[1] ^ s1xf1, true, chunk7,
...
void mitm_stage2b(archive_info& info, stage1_candidate& c1, vector<vector<stage2a>>& table, vector<stage2_candidate>& candidates, vector<vector<uint16_t>>& preimages, correct_guess *c) {
            ...
            for (auto t : thirds) {
              ...
              for (auto c2 : table2[mapkey]) {
                ...
                for (auto k20 : c1.maybek20) {
                  ...
                  if ((pxf0 & 0x3f) == ((crc32(k21xf0, c2.msbk12xf0 >> 24) >> 2) & 0x3f)) {
