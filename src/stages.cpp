...
namespace breakzip {
    ...
    guess_t stage2_correct_guess(const crack_t crypt_test) {
        ...
        for (auto file: zip.files) {
            ...
            const uint32_t low24_key10cc2 = (k10 * CRYPTCONST_POW2) & 0xffffff;

            // Compute carry bit for x.
            const uint32_t low24_lsb_key01xcc2 =
                (LSB(key01x) * CRYPTCONST_POW2) & 0xffffff;
            const uint32_t carry_bit_xtemp = low24_key10cc2 + low24_lsb_key01xcc2 +
            ...
            const uint8_t msb_key11x = temp + chunk3 +
            ...
            // TODO(stay): Mike, how do I calculate carry_for_y here?
            uint8_t msb_key11y = (uint8_t) (tt + chunk3 + carry_for_y);
            const uint32_t key20_low24bits = (chunk4 << 16) | chunk1;
            const uint32_t key21y_low24bits = crc32(key20_low24bits, msb_key11y);
            const uint32_t ttt = key21y_low24bits | 3;
            const uint8_t s1y = ((ttt * (ttt ^ 1)) >> 8) & 0xff;

            // Compute carry bit for y.
            const uint32_t low24_lsb_key01ycc2 =
                (LSB(key01y) * CRYPTCONST_POW2) & 0xffffff;
            const uint32_t carry_bit_ytemp = low24_key10cc2 + low24_lsb_key01ycc2 +
        ...
        guess_t rval(chunk1, chunk2, chunk3, chunk4, chunk5, chunk6, chunk7,
                carry_bits);
        return rval;
    }
        ...
        for (auto stage1_guess: in) {
            ...
            for (auto guess: stage2_range(*state, stage1_guess)) {
                ...
                for (auto file: zip.files) {
                    ...
                    const uint16_t s0 = get_s0(guess.chunk1);
                    const uint8_t y0 = x0 ^ s0;
                    const uint32_t key20 =
                        guess.chunk1 | (guess.chunk4 << 16) | (guess.chunk5 << 24);
                    const uint32_t key01x =
                        (guess.chunk2 | (guess.chunk6 << 8)) ^ crc32tab[x0];
                    ...
                    const uint8_t msb_key11x =
                        (uint8_t)(msbkey11x_temp + guess.chunk3 + stage1_carry_for_x);
                    ...
                    const uint32_t key01y =
                        (guess.chunk2 | (guess.chunk6 << 8)) ^ crc32tab[y0];
                    ...
                    const uint8_t msb_key11y =
                        (uint8_t)(msbkey11y_temp + guess.chunk3 + stage1_carry_for_y);
