...
HRESULT CheckRenderD3D9::SurfaceToPPM(IDirect3DDevice9 *pDevice,
    ...
    // This is required because we cannot lock a D3DPOOL_DEAULT surface
    // directly.  So, we copy to our sysmem surface.
    if (FAILED(pDevice->GetRenderTargetData(pSurface, pTargetSurface))) {
        printf(
            "Unable to GetRenderTargetData() for surface transfer! "
            "Aborting...\n");
        return E_FAIL;
    }
    ...
    //
    // $ PPM is BGR and not RGB it seems. Saved image looks "funny" in
    // viewer(red and blue swapped), but since ref will be dumped using same
    // method, this is ok.
