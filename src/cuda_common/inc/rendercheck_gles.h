...
public:
    ...
    virtual bool readback(GLuint width, GLuint height) {
        ...
        if (m_bUsePBO) {
            ...
            void *ioMem = glMapBufferRange(
                GL_PIXEL_PACK_BUFFER, 0, width * height * m_Bpp, GL_READ_ONLY);
...
public:
    ...
    GLuint createTexture(GLenum target, int w, int h, GLint internalformat,
        ...
        if (internalformat == GL_DEPTH_COMPONENT24) {
            glTexImage2D(target, 0, internalformat, w, h, 0, format,
                         GL_UNSIGNED_INT, 0);
        } else {
            glTexImage2D(target, 0, internalformat, w, h, 0, format,
                         GL_UNSIGNED_BYTE, 0);
...
public:
    ...
    virtual bool readback(GLuint width, GLuint height) {
        ...
        if (m_bUsePBO) {
            ...
            if (!ret) {
                printf(
                    "CheckFBO::readback() glBindFramebuffer checkStatus = %d\n",
                    ret);
            }
