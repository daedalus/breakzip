...
void mitm_stage1b(archive_info& info, vector<vector<stage1a>>& table, vector<stage1_candidate>& candidates, vector<vector<uint16_t>>& preimages, correct_guess *c) {
          ...
          for (auto s : seconds) {
            for (auto t : thirds) {
              ...
              for (stage1a candidate : table[mapkey]) {
                ...
                g.cb1 = candidate.cb1;
                g.m1 = (candidate.msbk11xf0 * 0x01010101) ^ mapkey;
                ...
                for (uint8_t i = 0; i < 64; ++i) {
                  ...
                  for (uint8_t lo = 0; lo < 4; ++lo) {
                    ...
                    if ((match & 0xfc) == (crck20 & 0xfc)) {
                      // KL ^ GH = BC.  (B = BC >> 8) & 0xff.
                      uint8_t b = ((crck20 ^ crc32tab[maybek20 & 0xff]) >> 8) & 0xff;
                      // BCD = (B << 16) | CD
                      g.maybek20.push_back((b << 16) | maybek20);
                    }
                  }
                }
