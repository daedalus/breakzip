    ...
    for (uint16_t chunk2 = 0; chunk2 < 0x100; ++chunk2) {
      for (uint16_t chunk3 = 0; chunk3 < 0x100; ++chunk3) {
        for (uint8_t carries = 0; carries < 0x10; ++carries) {
          if (nullptr != c && s0 == c->sx[0][0] && chunk2 == c->chunk2 && chunk3 == c->chunk3 &&
          ...
          for (auto s : seconds) {
            for (auto t : thirds) {
              ...
              for (stage1a candidate : table1[mapkey]) {
                ...
                // Get ~4 possible solutions for lo24(k20) = chunks 1 and 4
                //       A  B  C  D   k20
                // ^  E  F  G  H      crc32tab[D]
                //    ----------
                //    I  J  K  L      crck20
                ...
                // Starting at the bottom, derive 15..2 of KL from 
                // 15..2 of ST and OP
                uint16_t crck20 =
                    ((pxf0 << 2) ^ crc32tab[candidate.msbk11xf0]) & 0xfffc;
                ...
                for (uint8_t i = 0; i < 64; ++i) {
                  ...
                  for (uint8_t lo = 0; lo < 4; ++lo) {
                    ...
                    // L' = C ^ H
                    uint8_t match = (maybek20 >> 8) ^ crc32tab[maybek20 & 0xff];
                    // If upper six bits of L == upper six of L' then we have a candidate
                    if ((match & 0xfc) == (crck20 & 0xfc)) {
                      // KL ^ GH = BC.  (BC << 8) | CD = BCD.
                      g.maybek20.push_back(((crck20 ^ crc32tab[maybek20 & 0xff])<<8) | maybek20);
                    }
                  }
                }
